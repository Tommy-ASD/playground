...
impl EventHandler for Handler {
    ...
    async fn message(&self, ctx: Context, msg: Message) {
        ...
        if msg.content == "!join" {
            ...
            // of permissions to post in the channel, so log to stdout when some error happens,
            // with a description of it.
            if let Err(why) = join_inner(&ctx, &msg).await {
                println!("Error sending message: {why:?}");
            }
        }
        if msg.content.starts_with("!play") {
            ...
            // of permissions to post in the channel, so log to stdout when some error happens,
            // with a description of it.
            if let Err(why) = play_inner(&ctx, &msg, url.to_string()).await {
                println!("Error sending message: {why:?}");
            }
        }
    }
...
async fn play(ctx: &Context, msg: &Message, mut args: Args) -> CommandResult {
    ...
    play_inner(ctx, msg, url).await
}

async fn play_inner(ctx: &Context, msg: &Message, url: String) -> CommandResult {
    let guild_id = msg.guild_id.unwrap();
