...
pub fn oauth_router() -> Router {
    let state = AppState {
        ...
        key: Key::from(PRIVATE_KEY),
    };
    Router::new()
        .route("/callback/:name", get(oauth_callback))
        .route("/authenticate/:name", get(oauth_authenticate))
        .with_state(state)
}
...
pub async fn oauth_callback(
    ...
    jar.iter()
        .enumerate()
        .for_each(|(index, cookie)| println!("Cookie {index}: {cookie:?}"));

    // jar.add(Cookie::named("token"))
    (jar, Redirect::to("/"))
}
...
pub async fn oauth_authenticate(
    ...
    if let Some(csrf) = jar.get("csrf_state") {
    } else {
    }
