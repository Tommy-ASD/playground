...
impl From<ConnectionStatus> for JsValue {
...
impl From<Message> for JsValue {
    fn from(value: Message) -> Self {
        match value {
            Message::Text(tx) => return JsValue::from_str(&tx),
            Message::Binary(bin) => return JsValue::from_serde(&bin).unwrap(),
        }
    }
}
...
impl EventClient {
    pub fn get_status(&self) -> String {
        self.status.try_borrow().unwrap().to_string()
    }
