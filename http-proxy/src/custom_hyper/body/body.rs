...
use crate::custom_hyper::common::sync_wrapper::SyncWrapper;
use crate::custom_hyper::common::Future;
use crate::custom_hyper::common::Never;
use crate::custom_hyper::common::{task, watch, Pin, Poll};
use crate::custom_hyper::proto::h2::ping;

type BodySender = mpsc::Sender<Result<Bytes, crate::custom_hyper::Error>>;
type TrailersSender = oneshot::Sender<HeaderMap>;

/// A stream of `Bytes`, used when receiving bodies.
///
/// A good default [`HttpBody`](crate::custom_hyper::body::HttpBody) to use in many
/// applications.
///
/// Note: To read the full body, use [`body::to_bytes`](crate::custom_hyper::body::to_bytes())
/// or [`body::aggregate`](crate::custom_hyper::body::aggregate()).
...
enum Kind {
    ...
    Chan {
        ...
        data_rx: mpsc::Receiver<Result<Bytes, crate::custom_hyper::Error>>,
    ...
    Ffi(crate::custom_hyper::ffi::UserBody),
    #[cfg(feature = "stream")]
    Wrapped(
        SyncWrapper<
...
impl Body {
    ...
    /// `Cargo.toml`.
    pub(crate) fn wrap_stream<S, O, E>(stream: S) -> Body
    where
    ...
    ) -> Poll<Option<crate::custom_hyper::Result<Bytes>>> {
    ...
    pub(crate) fn as_ffi_mut(&mut self) -> &mut crate::custom_hyper::ffi::UserBody {
        match self.kind {
            Kind::Ffi(ref mut body) => return body,
            _ => {
                self.kind = Kind::Ffi(crate::custom_hyper::ffi::UserBody::new());
            }
        }
    ...
    fn poll_inner(
        ...
        match self.kind {
            ...
            Kind::H2 {
                ...
                Some(Err(e)) => Poll::Ready(Some(Err(crate::custom_hyper::Error::new_body(e)))),
                None => Poll::Ready(None),
            },
            ...
            Kind::Wrapped(ref mut s) => match ready!(s.get_mut().as_mut().poll_next(cx)) {
                Some(res) => Poll::Ready(Some(res.map_err(crate::custom_hyper::Error::new_body))),
                None => Poll::Ready(None),
            },
        }
    }
...
impl HttpBody for Body {
    type Data = Bytes;
    type Error = crate::custom_hyper::Error;
    ...
    fn poll_trailers(
        ...
        match self.kind {
            ...
            Kind::H2 {
                ...
                Err(e) => Poll::Ready(Err(crate::custom_hyper::Error::new_h2(e))),
            },
            Kind::Chan {
                ref mut trailers_rx,
                ..
...
impl Stream for Body {
    type Item = crate::custom_hyper::Result<Bytes>;
...
impl Sender {
    ...
    pub(crate) fn poll_ready(
        ...
        self.data_tx
            .poll_ready(cx)
            .map_err(|_| crate::custom_hyper::Error::new_closed())
    }

    fn poll_want(&mut self, cx: &mut task::Context<'_>) -> Poll<crate::custom_hyper::Result<()>> {
        match self.want_rx.load(cx) {
            WANT_READY => Poll::Ready(Ok(())),
            WANT_PENDING => Poll::Pending,
            watch::CLOSED => Poll::Ready(Err(crate::custom_hyper::Error::new_closed())),
            unexpected => unreachable!("want_rx value: {}", unexpected),
        }
    }

    async fn ready(&mut self) -> crate::custom_hyper::Result<()> {
        futures_util::future::poll_fn(|cx| self.poll_ready(cx)).await
    }

    /// Send data on data channel when it is ready.
    pub(crate) async fn send_data(&mut self, chunk: Bytes) -> crate::custom_hyper::Result<()> {
        self.ready().await?;
        self.data_tx
            .try_send(Ok(chunk))
            .map_err(|_| crate::custom_hyper::Error::new_closed())
    ...
    pub(crate) async fn send_trailers(
        ...
        let tx = match self.trailers_tx.take() {
            Some(tx) => tx,
            None => return Err(crate::custom_hyper::Error::new_closed()),
        };
        tx.send(trailers)
            .map_err(|_| crate::custom_hyper::Error::new_closed())
    }
    ...
    pub(crate) fn abort(mut self) {
        self.send_error(crate::custom_hyper::Error::new_body_write_aborted());
    }

    pub(crate) fn send_error(&mut self, err: crate::custom_hyper::Error) {
        let _ = self
            .data_tx
