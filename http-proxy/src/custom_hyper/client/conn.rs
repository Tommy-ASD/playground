...
pub(crate) async fn handshake<T>(
    ...
    SendRequest<crate::custom_hyper::Body>,
    Connection<T, crate::custom_hyper::Body>,
)>
where
    ...
    pub(crate) fn send_request(&mut self, req: Request<B>) -> ResponseFuture {
        let inner = match self.dispatch.send(req) {
            ...
            Err(_req) => {
                ...
                let err =
                    crate::custom_hyper::Error::new_canceled().with("connection was not ready");
    ...
    B::Error: Into<Box<dyn StdError + Send + Sync>>,
{
    ...
    pub(crate) fn without_shutdown(
        self,
    ) -> impl Future<Output = crate::custom_hyper::Result<Parts<T>>> {
        let mut conn = Some(self);
        future::poll_fn(move |cx| -> Poll<crate::custom_hyper::Result<Parts<T>>> {
...
impl Builder {
    ...
    pub(crate) fn http1_read_buf_exact_size(&mut self, sz: Option<usize>) -> &mut Builder {
