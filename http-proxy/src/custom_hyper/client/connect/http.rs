...
use crate::custom_tokio::net::{TcpSocket, TcpStream};
...
// use super::dns::TokioThreadpoolGaiResolver;
...
impl HttpConnector<TokioThreadpoolGaiResolver> {
    ...
    pub(crate) fn new_with_tokio_threadpool_resolver() -> Self {
...
impl<R> HttpConnector<R> {
    /// Construct a new HttpConnector.
    ///
    /// Takes a [`Resolver`](crate::custom_hyper::client::connect::dns#resolvers-are-services) to handle DNS lookups.
        ...
        if let Some(fallback_timeout) = config.happy_eyeballs_timeout {
            ...
            ConnectingTcp {
                ...
                fallback: Some(ConnectingTcpFallback {
                    delay: crate::custom_tokio::time::sleep(fallback_timeout),
...
fn connect(
    ...
    Ok(async move {
        match connect_timeout {
            Some(dur) => match crate::custom_tokio::time::timeout(dur, connect).await {
                Ok(Ok(s)) => Ok(s),
                Ok(Err(e)) => Err(e),
