...
use crate::custom_hyper::common::Pin;
use crate::custom_hyper::common::{task, Poll};

pub(crate) type RetryPromise<T, U> =
    oneshot::Receiver<Result<U, (crate::custom_hyper::Error, Option<T>)>>;
pub(crate) type Promise<T> = oneshot::Receiver<Result<T, crate::custom_hyper::Error>>;
...
impl<T, U> Drop for Envelope<T, U> {
    fn drop(&mut self) {
        if let Some((val, cb)) = self.0.take() {
            cb.send(Err((
                crate::custom_hyper::Error::new_canceled().with("connection closed"),
                Some(val),
            )));
        }
    }
...
pub(crate) enum Callback<T, U> {
    Retry(Option<oneshot::Sender<Result<U, (crate::custom_hyper::Error, Option<T>)>>>),
    NoRetry(Option<oneshot::Sender<Result<U, crate::custom_hyper::Error>>>),
}

impl<T, U> Drop for Callback<T, U> {
    fn drop(&mut self) {
        // FIXME(nox): What errors do we want here?
        let error = crate::custom_hyper::Error::new_user_dispatch_gone().with(
...
impl<T, U> Callback<T, U> {
    ...
    pub(crate) fn send(mut self, val: Result<U, (crate::custom_hyper::Error, Option<T>)>) {
        match self {
    ...
    pub(crate) async fn send_when(
        self,
        mut when: impl Future<Output = Result<U, (crate::custom_hyper::Error, Option<T>)>> + Unpin,
    ) {
