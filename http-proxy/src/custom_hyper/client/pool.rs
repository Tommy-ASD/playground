...
use crate::custom_tokio::time::{Duration, Instant, Interval};
use futures_channel::oneshot;
use tracing::{debug, trace};

use super::client::Ver;
use crate::custom_hyper::common::{exec::Exec, task, Future, Pin, Poll, Unpin};
...
struct PoolInner<T> {
    ...
    idle_interval_ref: Option<oneshot::Sender<crate::custom_hyper::common::Never>>,
...
impl<T: Poolable> PoolInner<T> {
    ...
    fn spawn_idle_interval(&mut self, pool_ref: &Arc<Mutex<PoolInner<T>>>) {
        ...
        let interval = IdleTask {
            interval: crate::custom_tokio::time::interval(dur),
...
impl<T: Poolable> Checkout<T> {
    ...
    ) -> Poll<Option<crate::custom_hyper::Result<Pooled<T>>>> {
            ...
            match Pin::new(&mut rx).poll(cx) {
                Poll::Ready(Ok(value)) => {
                    if value.is_open() {
                        ...
                        Poll::Ready(Some(Err(
                            crate::custom_hyper::Error::new_canceled().with(CheckoutIsClosedError)
                        )))
                    }
                }
                Poll::Pending => {
                ...
                Poll::Ready(Err(_canceled)) => {
                    Poll::Ready(Some(Err(crate::custom_hyper::Error::new_canceled()
                        .with("request has been canceled"))))
                }
            }
        } else {
            Poll::Ready(None)
...
impl<T: Poolable> Future for Checkout<T> {
    type Output = crate::custom_hyper::Result<Pooled<T>>;
    ...
    fn poll(mut self: Pin<&mut Self>, cx: &mut task::Context<'_>) -> Poll<Self::Output> {
        ...
        if let Some(pooled) = self.checkout(cx) {
            ...
            Poll::Ready(Err(
                crate::custom_hyper::Error::new_canceled().with("pool is disabled")
            ))
        } else {
...
pin_project_lite::pin_project! {
    struct IdleTask<T> {
        ...
        #[pin]
        pool_drop_notifier: oneshot::Receiver<crate::custom_hyper::common::Never>,
    }
}
...
mod tests {
    ...
    use crate::custom_hyper::common::{exec::Exec, task, Future, Pin};
    ...
    async fn test_pool_checkout_returns_none_if_expired() {
        ...
        drop(pooled);
        crate::custom_tokio::time::sleep(pool.locked().timeout.unwrap()).await;
    ...
    async fn test_pool_timer_removes_expired() {
        ...
        crate::custom_tokio::time::advance(Duration::from_millis(30)).await;
