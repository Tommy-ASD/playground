...
use crate::custom_hyper::body::{Body, Bytes, HttpBody as _};
        ...
        Box::into_raw(hyper_task::boxed(async move {
            while let Some(item) = body.0.data().await {
                ...
                if HYPER_ITER_CONTINUE != func(userdata.0, &hyper_buf(chunk)) {
                    return Err(crate::custom_hyper::Error::new_user_aborted_by_callback());
                }
            }
            Ok(())
        }))
...
impl UserBody {
    ...
    pub(crate) fn poll_data(
        ...
        match (self.data_func)(self.userdata, hyper_context::wrap(cx), &mut out) {
            ...
            super::task::HYPER_POLL_ERROR => Poll::Ready(Some(Err(
                crate::custom_hyper::Error::new_body_write_aborted(),
            ))),
            unexpected => Poll::Ready(Some(Err(crate::custom_hyper::Error::new_body_write(
                format!("unexpected hyper_body_data_func return code {}", unexpected),
            )))),
        }
    }
    ...
    pub(crate) fn poll_trailers(
        &mut self,
        _cx: &mut Context<'_>,
    ) -> Poll<crate::custom_hyper::Result<Option<HeaderMap>>> {
        Poll::Ready(Ok(None))
    }
}
