...
use crate::custom_hyper::client::connect::{Connect, Connected, Destination};
...
impl DuplexHandle {
    pub(crate) fn read(&self, buf: &mut [u8]) -> Poll<usize, io::Error> {
        let mut inner = self.inner.lock().unwrap();
    ...
    pub(crate) fn write(&self, bytes: &[u8]) -> Poll<usize, io::Error> {
        let mut inner = self.inner.lock().unwrap();
...
impl MockConnector {
    pub(crate) fn new() -> MockConnector {
        MockConnector {
            mocks: Arc::new(Mutex::new(MockedConnections(HashMap::new()))),
        }
    }

    pub(crate) fn mock(&mut self, key: &str) -> DuplexHandle {
        use futures::future;
        self.mock_fut(key, future::ok::<_, ()>(()))
    }

    pub(crate) fn mock_fut<F>(&mut self, key: &str, fut: F) -> DuplexHandle
    where
    ...
    pub(crate) fn mock_opts<F>(&mut self, key: &str, connected: Connected, fut: F) -> DuplexHandle
    where
        F: Future + Send + 'static,
    {
