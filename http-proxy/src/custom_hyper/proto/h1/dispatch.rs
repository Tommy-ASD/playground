...
use crate::custom_hyper::body::{Body, DecodedLength, HttpBody};
use crate::custom_hyper::common::{task, Future, Pin, Poll, Unpin};
use crate::custom_hyper::proto::{BodyLength, Conn, Dispatched, MessageHead, RequestHead};
use crate::custom_hyper::upgrade::OnUpgrade;

pub(crate) struct Dispatcher<D, Bs: HttpBody, I, T> {
    conn: Conn<I, Bs::Data, T>,
    dispatch: D,
    body_tx: Option<crate::custom_hyper::body::Sender>,
...
pub(crate) trait Dispatch {
    ...
    fn recv_msg(
        &mut self,
        msg: crate::custom_hyper::Result<(Self::RecvItem, Body)>,
    ) -> crate::custom_hyper::Result<()>;
...
pin_project_lite::pin_project! {
    pub(crate) struct Client<B> {
        callback: Option<crate::custom_hyper::client::dispatch::Callback<Request<B>, http::Response<Body>>>,
        #[pin]
        rx: ClientRx<B>,
        rx_closed: bool,
    }
...
type ClientRx<B> =
    crate::custom_hyper::client::dispatch::Receiver<Request<B>, http::Response<Body>>;
    ...
    Bs::Error: Into<Box<dyn StdError + Send + Sync>>,
{
    ...
    ) -> Poll<crate::custom_hyper::Result<Dispatched>> {
        Poll::Ready(ready!(self.poll_inner(cx, should_shutdown)).or_else(|e| {
            // Be sure to alert a streaming body of the failure.
            if let Some(mut body) = self.body_tx.take() {
                body.send_error(crate::custom_hyper::Error::new_body("connection error"));
            }
        ...
        if self.is_done() {
            if let Some(pending) = self.conn.pending_upgrade() {
                ...
                ready!(self.conn.poll_shutdown(cx))
                    .map_err(crate::custom_hyper::Error::new_shutdown)?;
            }
            self.conn.take_error()?;
    ...
    fn poll_loop(&mut self, cx: &mut task::Context<'_>) -> Poll<crate::custom_hyper::Result<()>> {
    ...
    fn poll_read(&mut self, cx: &mut task::Context<'_>) -> Poll<crate::custom_hyper::Result<()>> {
        loop {
            if self.is_closing {
                return Poll::Ready(Ok(()));
                ...
                if self.conn.can_read_body() {
                    ...
                    match self.conn.poll_read_body(cx) {
                        ...
                        Poll::Ready(Some(Err(e))) => {
                            body.send_error(crate::custom_hyper::Error::new_body(e));
                        }
                    }
                } else {
                    // just drop, the body will close automatically
    ...
    fn poll_write(&mut self, cx: &mut task::Context<'_>) -> Poll<crate::custom_hyper::Result<()>> {
        loop {
            if self.is_closing {
                return Poll::Ready(Ok(()));
                ...
                && self.dispatch.should_poll()
            {
                if let Some(msg) = ready!(Pin::new(&mut self.dispatch).poll_msg(cx)) {
                    let (head, mut body) =
                        msg.map_err(crate::custom_hyper::Error::new_user_service)?;

                    // Check if the body knows its full data immediately.
                    //
                    // If so, we can skip a bit of bookkeeping that streaming
                    // bodies need to do.
                    if let Some(full) = crate::custom_hyper::body::take_full_data(&mut body) {
                    ...
                    OptGuard::new(self.body_rx.as_mut()).guard_mut()
                {
                    ...
                    if let Some(item) = item {
                        let chunk = item.map_err(|e| {
                            *clear_body = true;
                            crate::custom_hyper::Error::new_user_body(e)
                        })?;
    ...
    fn poll_flush(&mut self, cx: &mut task::Context<'_>) -> Poll<crate::custom_hyper::Result<()>> {
        self.conn.poll_flush(cx).map_err(|err| {
            debug!("error writing: {}", err);
            crate::custom_hyper::Error::new_body_write(err)
        })
    }
...
where
    ...
    Bs::Error: Into<Box<dyn StdError + Send + Sync>>,
{
    type Output = crate::custom_hyper::Result<Dispatched>;
    ...
    B: HttpBody,
{
    ...
    type PollBody = B;
    type PollError = crate::custom_hyper::common::Never;
    type RecvItem = crate::custom_hyper::proto::ResponseHead;

    fn poll_msg(
        mut self: Pin<&mut Self>,
        cx: &mut task::Context<'_>,
    ) -> Poll<Option<Result<(Self::PollItem, Self::PollBody), crate::custom_hyper::common::Never>>>
    {
        let mut this = self.as_mut();
                ...
                match cb.poll_canceled(cx) {
                    ...
                    Poll::Pending => {
                        ...
                        let head = RequestHead {
                            version: parts.version,
                            subject: crate::custom_hyper::proto::RequestLine(
                                parts.method,
                                parts.uri,
                            ),
                            headers: parts.headers,
        ...
        match msg {
            Ok((msg, body)) => {
                if let Some(cb) = self.callback.take() {
                    ...
                    // full message!
                    Err(crate::custom_hyper::Error::new_unexpected_message())
                }
            }
            Err(err) => {
                if let Some(cb) = self.callback.take() {
                    ...
                    if let Some((req, cb)) = self.rx.try_recv() {
                        ...
                        cb.send(Err((
                            crate::custom_hyper::Error::new_canceled().with(err),
                            Some(req),
                        )));
                        Ok(())
                    } else {
...
mod tests {
    use super::*;
    use crate::custom_hyper::proto::h1::ClientTransaction;
    ...
    fn client_read_bytes_before_writing_request() {
        ...
        tokio_test::task::spawn(()).enter(|cx, _| {
            ...
            let (mut tx, rx) = crate::custom_hyper::client::dispatch::channel();
            ...
            let mut res_rx = tx
                .try_send(crate::custom_hyper::Request::new(
                    crate::custom_hyper::Body::empty(),
                ))
                .unwrap();
            ...
            match (err.0.kind(), err.1) {
                (&crate::custom_hyper::error::Kind::Canceled, Some(_)) => (),
    ...
    async fn client_flushing_is_not_ready_for_next_request() {
        ...
        let _dispatcher = crate::custom_tokio::spawn(async move { dispatcher.await });

        let req = crate::custom_hyper::Request::builder()
            .method("POST")
            .body(crate::custom_hyper::Body::from("reee"))
            .unwrap();
    ...
    async fn body_empty_chunks_ignored() {
        ...
        let body = {
            let (mut tx, body) = crate::custom_hyper::Body::channel();
            tx.try_send_data("".into()).unwrap();
            body
        };

        let _res_rx = tx
            .try_send(crate::custom_hyper::Request::new(body))
            .unwrap();
