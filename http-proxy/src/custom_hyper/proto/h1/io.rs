...
use crate::custom_hyper::common::buf::BufList;
    ...
    B: Buf,
{
    ...
    ) -> Poll<crate::custom_hyper::Result<ParsedMessage<S::Incoming>>>
    where
        S: Http1Transaction,
    {
        loop {
            match super::role::parse_headers::<S>(
                ...
                None => {
                    ...
                    if self.read_buf.len() >= max {
                        ...
                        return Poll::Ready(Err(crate::custom_hyper::Error::new_too_large()));
                    }
                            ...
                            if Pin::new(h1_header_read_timeout_fut).poll(cx).is_ready() {
                                *parse_ctx.h1_header_read_timeout_running = false;
                                ...
                                return Poll::Ready(Err(
                                    crate::custom_hyper::Error::new_header_timeout(),
                                ));
                            }
                        }
                    }
            ...
            if ready!(self.poll_read_from_io(cx)).map_err(crate::custom_hyper::Error::new_io)? == 0
            {
                trace!("parse eof");
                return Poll::Ready(Err(crate::custom_hyper::Error::new_incomplete()));
            }
        }
    }
