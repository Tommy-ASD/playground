...
use crate::custom_tokio::time::Instant;
use bytes::Bytes;
use bytes::BytesMut;
...
use crate::custom_hyper::body::DecodedLength;

use crate::custom_hyper::common::date;
use crate::custom_hyper::error::Parse;
use crate::custom_hyper::ext::HeaderCaseMap;

use crate::custom_hyper::ext::OriginalHeaderOrder;
use crate::custom_hyper::headers;
use crate::custom_hyper::proto::h1::{
    Encode, Encoder, Http1Transaction, ParseContext, ParseResult, ParsedMessage,
};
use crate::custom_hyper::proto::{BodyLength, MessageHead, RequestHead, RequestLine};
            ...
            match ctx.h1_header_read_timeout_fut {
                ...
                None => {
                    ...
                    *ctx.h1_header_read_timeout_fut =
                        Some(Box::pin(crate::custom_tokio::time::sleep_until(deadline)));
                }
            }
        }
    }
...
pub(super) fn encode_headers<T>(
    ...
    dst: &mut Vec<u8>,
) -> crate::custom_hyper::Result<Encoder>
where
    T: Http1Transaction,
{
...
impl Http1Transaction for Server {
    ...
    fn encode(
        ...
        let (ret, is_last) = if msg.head.subject == StatusCode::SWITCHING_PROTOCOLS {
            ...
            *msg.head = MessageHead::default();
            ...
            msg.body = None;
            (
                Err(crate::custom_hyper::Error::new_user_unsupported_status_code()),
                true,
            )
        } else {
            (Ok(()), !msg.keep_alive)
        ...
        let custom_reason_phrase = msg
            .head
            .extensions
            .get::<crate::custom_hyper::ext::ReasonPhrase>();
    ...
    fn on_error(err: &crate::custom_hyper::Error) -> Option<MessageHead<Self::Outgoing>> {
        use crate::custom_hyper::error::Kind;
            ...
            match *name {
                header::CONTENT_LENGTH => {
                    if wrote_len && !is_name_written {
                        ...
                        rewind(dst);
                        return Err(crate::custom_hyper::Error::new_user_header());
                    }
                    match msg.body {
...
impl Http1Transaction for Client {
    ...
    fn parse(buf: &mut BytesMut, ctx: ParseContext<'_>) -> ParseResult<StatusCode> {
        ...
        loop {
            ...
            if let Some(reason) = reason {
                ...
                // field.
                let reason =
                    unsafe { crate::custom_hyper::ext::ReasonPhrase::from_bytes_unchecked(reason) };
                extensions.insert(reason);
            }

            if ctx.raw_headers {
                extensions.insert(crate::custom_hyper::ffi::RawHeaders(
                    crate::custom_hyper::ffi::hyper_buf(slice),
                ));
            }
            ...
            if head.subject.is_informational() {
                if let Some(callback) = ctx.on_informational {
                    callback.call(head.into_response(crate::custom_hyper::Body::empty()));
                }
            }
    ...
    fn on_error(_err: &crate::custom_hyper::Error) -> Option<MessageHead<Self::Outgoing>> {
...
) -> Result<(), crate::custom_hyper::error::Parse> {
    let bytes_ptr = bytes.as_ptr() as usize;

    for (header, indices) in headers.iter().zip(indices.iter_mut()) {
        if header.name.len() >= (1 << 16) {
            debug!("header name larger than 64kb: {:?}", header.name);
            return Err(crate::custom_hyper::error::Parse::TooLarge);
        }
...
mod tests {
    ...
    fn test_parse_request() {
        ...
        .unwrap();
        assert_eq!(raw.len(), 0);
        assert_eq!(msg.head.subject.0, crate::custom_hyper::Method::GET);
        assert_eq!(msg.head.subject.1, "/echo");
        assert_eq!(msg.head.version, crate::custom_hyper::Version::HTTP_11);
        assert_eq!(msg.head.headers.len(), 1);
        assert_eq!(msg.head.headers["Host"], "hyper.rs");
        assert_eq!(method, Some(crate::custom_hyper::Method::GET));
    }
    ...
    fn test_parse_response() {
        ...
        let ctx = ParseContext {
            cached_headers: &mut None,
            req_method: &mut Some(crate::custom_hyper::Method::GET),
            h1_parser_config: Default::default(),
        ...
        assert_eq!(raw.len(), 0);
        assert_eq!(msg.head.subject, crate::custom_hyper::StatusCode::OK);
        assert_eq!(msg.head.version, crate::custom_hyper::Version::HTTP_11);
        assert_eq!(msg.head.headers.len(), 1);
    ...
    fn test_parse_response_h09_allowed() {
        ...
        assert_eq!(msg.head.subject, crate::custom_hyper::StatusCode::OK);
        assert_eq!(msg.head.version, crate::custom_hyper::Version::HTTP_09);
    ...
    fn test_decoder_request() {
        ...
        fn parse_err(s: &str, comment: &str) -> crate::custom_hyper::error::Parse {
            let mut bytes = BytesMut::from(s);
            Server::parse(
                &mut bytes,
                ParseContext {
    ...
    fn test_decoder_response() {
        ...
        fn parse_err(s: &str) -> crate::custom_hyper::error::Parse {
            let mut bytes = BytesMut::from(s);
            Client::parse(
                &mut bytes,
                ParseContext {
    ...
    fn test_client_request_encode_title_case() {
        use crate::custom_hyper::proto::BodyLength;
