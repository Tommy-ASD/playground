...
use crate::custom_hyper::client::dispatch::Callback;
use crate::custom_hyper::common::{exec::Exec, task, Future, Never, Pin, Poll};
use crate::custom_hyper::ext::Protocol;
use crate::custom_hyper::headers;
use crate::custom_hyper::proto::h2::UpgradedSendStream;
use crate::custom_hyper::proto::Dispatched;
use crate::custom_hyper::upgrade::Upgraded;
use crate::custom_hyper::{Body, Request, Response};
use h2::client::ResponseFuture;

type ClientRx<B> = crate::custom_hyper::client::dispatch::Receiver<Request<B>, Response<Body>>;
...
pub(crate) async fn handshake<T, B>(
    ...
    config: &Config,
    exec: Exec,
) -> crate::custom_hyper::Result<ClientTask<B>>
where
    ...
    fn poll_pipe(&mut self, f: FutCtx<B>, cx: &mut task::Context<'_>) {
        ...
        let fut = f.fut.map(move |result| match result {
            Ok(res) => {
                ...
                if let (Some(mut send_stream), StatusCode::OK) = (send_stream, res.status()) {
                    if content_length.map_or(false, |len| len != 0) {
                        ...
                        return Err((
                            crate::custom_hyper::Error::new_h2(h2::Reason::INTERNAL_ERROR.into()),
                            None,
                        ));
                    }
                    ...
                    let (pending, on_upgrade) = crate::custom_hyper::upgrade::pending();
                    let io = H2Upgraded {
                        ping,
                    ...
                    let res = res.map(|stream| {
                        ...
                        crate::custom_hyper::Body::h2(stream, content_length.into(), ping)
                    });
                    Ok(res)
                }
            }
            Err(err) => {
                ...
                Err((crate::custom_hyper::Error::new_h2(err), None))
            }
        });
            ...
            match ready!(self.h2_tx.poll_ready(cx)) {
                ...
                Err(err) => {
                    ...
                    return if err.reason() == Some(::h2::Reason::NO_ERROR) {
                        ...
                        Poll::Ready(Err(crate::custom_hyper::Error::new_h2(err)))
                    };
                }
            };
                ...
                Poll::Ready(Some((req, cb))) => {
                    ...
                    if is_connect {
                            ...
                            .map_or(false, |len| len != 0)
                        {
                            ...
                            cb.send(Err((
                                crate::custom_hyper::Error::new_h2(
                    ...
                    let (fut, body_tx) = match self.h2_tx.send_request(req, !is_connect && eos) {
                        ...
                        Err(err) => {
                            ...
                            cb.send(Err((crate::custom_hyper::Error::new_h2(err), None)));
                            continue;
                        }
                    };
                    ...
                    match self.h2_tx.poll_ready(cx) {
                        ...
                        Poll::Ready(Ok(())) => (),
                        Poll::Ready(Err(err)) => {
                            f.cb.send(Err((crate::custom_hyper::Error::new_h2(err), None)));
                            continue;
                        }
                    }
                    self.poll_pipe(f, cx);
