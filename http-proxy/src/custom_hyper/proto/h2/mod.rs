        ...
        loop {
            if !*me.data_done {
                ...
                if me.body_tx.capacity() == 0 {
                        ...
                        match ready!(me.body_tx.poll_capacity(cx)) {
                            ...
                            Some(Err(e)) => {
                                return Poll::Ready(Err(
                                    crate::custom_hyper::Error::new_body_write(e),
                                ))
                            }
                            None => {
                                ...
                                return Poll::Ready(Err(
                                    crate::custom_hyper::Error::new_body_write(
                    ...
                    .body_tx
                    .poll_reset(cx)
                    .map_err(crate::custom_hyper::Error::new_body_write)?
                {
                    debug!("stream received RST_STREAM: {:?}", reason);
                    return Poll::Ready(Err(crate::custom_hyper::Error::new_body_write(
...
trait SendStreamExt {
    fn on_user_err<E>(&mut self, err: E) -> crate::custom_hyper::Error
    where
        E: Into<Box<dyn std::error::Error + Send + Sync>>;
    fn send_eos_frame(&mut self) -> crate::custom_hyper::Result<()>;
}
...
impl<B: Buf> SendStreamExt for SendStream<SendBuf<B>> {
    ...
    where
        E: Into<Box<dyn std::error::Error + Send + Sync>>,
    {
        let err = crate::custom_hyper::Error::new_user_body(err);
