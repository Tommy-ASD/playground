...
use crate::custom_tokio::time::{Instant, Sleep};
...
pub(super) fn channel(ping_pong: PingPong, config: Config) -> (Recorder, Ponger) {
    ...
    let keep_alive = config.keep_alive_interval.map(|interval| KeepAlive {
        ...
        timer: Box::pin(crate::custom_tokio::time::sleep(interval)),
...
impl Recorder {
    ...
    pub(super) fn ensure_not_timed_out(&self) -> crate::custom_hyper::Result<()> {
        {
...
impl KeepAliveTimedOut {
    pub(super) fn crate_error(self) -> crate::custom_hyper::Error {
        crate::custom_hyper::Error::new(crate::custom_hyper::error::Kind::Http2).with(self)
    }
}
...
impl std::error::Error for KeepAliveTimedOut {
    fn source(&self) -> Option<&(dyn std::error::Error + 'static)> {
        Some(&crate::custom_hyper::error::TimedOut)
    }
}
