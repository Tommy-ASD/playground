...
use crate::custom_hyper::common::{date, task, Future, Pin, Poll};
use crate::custom_hyper::ext::Protocol;
use crate::custom_hyper::headers;
use crate::custom_hyper::proto::h2::ping::Recorder;
use crate::custom_hyper::proto::h2::{H2Upgraded, UpgradedSendStream};
use crate::custom_hyper::proto::Dispatched;
use crate::custom_hyper::service::HttpService;

use crate::custom_hyper::upgrade::{OnUpgrade, Pending, Upgraded};
use crate::custom_hyper::{Body, Response};
    ...
    B: HttpBody,
{
    ...
    closing: Option<crate::custom_hyper::Error>,
}
        ...
        loop {
            let next = match me.state {
                State::Handshaking {
                    ...
                    let mut conn = ready!(Pin::new(hs)
                        .poll(cx)
                        .map_err(crate::custom_hyper::Error::new_h2))?;
        ...
        if self.closing.is_none() {
            loop {
                ...
                match service.poll_ready(cx) {
                    ...
                    Poll::Ready(Err(err)) => {
                        let err = crate::custom_hyper::Error::new_user_service(err);
                ...
                match ready!(self.conn.poll_accept(cx)) {
                    Some(Ok((req, mut respond))) => {
                        ...
                        let (mut req, connect_parts) = if !is_connect {
                            ...
                            let (pending, upgrade) = crate::custom_hyper::upgrade::pending();
                            debug_assert!(parts.extensions.get::<OnUpgrade>().is_none());
                            parts.extensions.insert(upgrade);
                            (
                                Request::from_parts(parts, crate::custom_hyper::Body::empty()),
                                Some(ConnectParts {
                                    pending,
                                    ping,
                                    recv_stream: stream,
                    ...
                    Some(Err(e)) => {
                        return Poll::Ready(Err(crate::custom_hyper::Error::new_h2(e)));
                    }
                    None => {
                    ...
                    let res = match h.poll(cx) {
                        ...
                        Poll::Pending => {
                                ...
                                .map_err(crate::custom_hyper::Error::new_h2)?
                            {
                                ...
                                return Poll::Ready(Err(crate::custom_hyper::Error::new_h2(
                                    reason.into(),
                                )));
                            }
                            return Poll::Pending;
                        ...
                        Poll::Ready(Err(e)) => {
                            let err = crate::custom_hyper::Error::new_user_service(e);
                        ...
                        if res.status().is_success() {
                                ...
                                .map_or(false, |len| len != 0)
                            {
                                ...
                                return Poll::Ready(Err(
                                    crate::custom_hyper::Error::new_user_header(),
                                ));
                            }
