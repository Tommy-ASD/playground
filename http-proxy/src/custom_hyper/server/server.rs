...
use crate::custom_hyper::common::exec::Exec;
use crate::custom_hyper::common::exec::{ConnStreamExec, NewSvcExec};
...
use crate::custom_hyper::service::{HttpService, MakeServiceRef};
...
pub(crate) struct Builder<I, E = Exec> {
    incoming: I,
    protocol: Http_<E>,
}
...
impl<I> Server<I, ()> {
    ...
    pub(crate) fn builder(incoming: I) -> Builder<I> {
        Builder {
            incoming,
            protocol: Http_::new(),
        }
...
impl Server<AddrIncoming, ()> {
    ...
    pub(crate) fn bind(addr: &SocketAddr) -> Builder<AddrIncoming> {
    ...
    pub(crate) fn try_bind(
        addr: &SocketAddr,
    ) -> crate::custom_hyper::Result<Builder<AddrIncoming>> {
    ...
    ) -> Result<Builder<AddrIncoming>, crate::custom_hyper::Error> {
    ...
    E: ConnStreamExec<<S::Service as HttpService<Body>>::Future, B>,
{
    ...
    /// # }
    /// ```
    pub(crate) fn with_graceful_shutdown<F>(self, signal: F) -> Graceful<I, S, F, E>
    where
        F: Future<Output = ()>,
    ...
    ) -> Poll<Option<crate::custom_hyper::Result<Connecting<IO, S::Future, E>>>> {
        let me = self.project();
        match ready!(me.make_service.poll_ready_ref(cx)) {
            ...
            Err(e) => {
                ...
                return Poll::Ready(Some(Err(
                    crate::custom_hyper::Error::new_user_make_service(e),
                )));
            }
        }

        if let Some(item) = ready!(me.incoming.poll_accept(cx)) {
            let io = item.map_err(crate::custom_hyper::Error::new_accept)?;
...
impl<I, E> Builder<I, E> {
    ...
    pub(crate) fn http1_pipeline_flush(mut self, val: bool) -> Self {
    ...
    pub(crate) fn http2_initial_stream_window_size(mut self, sz: impl Into<Option<u32>>) -> Self {
    ...
    pub(crate) fn http2_max_concurrent_streams(mut self, max: impl Into<Option<u32>>) -> Self {
    ...
    pub(crate) fn http2_keep_alive_timeout(mut self, timeout: Duration) -> Self {
    ...
    ///
    /// Default is `crate::custom_tokio::spawn`.
    ...
    /// # }
    /// ```
    pub(crate) fn serve<S, B>(self, make_service: S) -> Server<I, S, E>
    where
        I: Accept,
...
pub(crate) trait Watcher<I, S: HttpService<Body>, E>: Clone {
    type Future: Future<Output = crate::custom_hyper::Result<()>>;
...
pub(crate) mod new_svc {
    ...
    use crate::custom_hyper::body::{Body, HttpBody};
    use crate::custom_hyper::common::exec::ConnStreamExec;
    use crate::custom_hyper::common::{task, Future, Pin, Poll, Unpin};
    ...
    pin_project! {
        #[allow(missing_debug_implementations)]
        pub(crate) struct NewSvcTask<I, N, S: HttpService<Body>, E, W: Watcher<I, S, E>> {
            #[pin]
                    ...
                    match me.state.as_mut().project() {
                        StateProj::Connecting {
                            ...
                            let conn = match res {
                                ...
                                Err(err) => {
                                    let err =
                                        crate::custom_hyper::Error::new_user_make_service(err);
...
pin_project! {
    ...
    pub(crate) struct Connecting<I, F, E = Exec> {
        #[pin]
        future: F,
        io: Option<I>,
        protocol: Http_<E>,
