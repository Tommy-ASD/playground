...
pub(crate) use self::addr_stream::AddrStream;
...
impl AddrIncoming {
    pub(super) fn new(addr: &SocketAddr) -> crate::custom_hyper::Result<Self> {
        let std_listener =
            StdTcpListener::bind(addr).map_err(crate::custom_hyper::Error::new_listen)?;

        AddrIncoming::from_std(std_listener)
    }

    pub(super) fn from_std(std_listener: StdTcpListener) -> crate::custom_hyper::Result<Self> {
        // TcpListener::from_std doesn't set O_NONBLOCK
        std_listener
            .set_nonblocking(true)
            .map_err(crate::custom_hyper::Error::new_listen)?;
        let listener =
            TcpListener::from_std(std_listener).map_err(crate::custom_hyper::Error::new_listen)?;
        AddrIncoming::from_listener(listener)
    }

    /// Creates a new `AddrIncoming` binding to provided socket address.
    pub(crate) fn bind(addr: &SocketAddr) -> crate::custom_hyper::Result<Self> {
        AddrIncoming::new(addr)
    }

    /// Creates a new `AddrIncoming` from an existing `crate::custom_tokio::net::TcpListener`.
    pub(crate) fn from_listener(listener: TcpListener) -> crate::custom_hyper::Result<Self> {
        let addr = listener
            .local_addr()
            .map_err(crate::custom_hyper::Error::new_listen)?;
        Ok(AddrIncoming {
    ...
    pub(crate) fn set_keepalive(&mut self, time: Option<Duration>) -> &mut Self {
    ...
    pub(crate) fn set_keepalive_interval(&mut self, interval: Option<Duration>) -> &mut Self {
        self.tcp_keepalive_config.interval = interval;
        self
    }

    /// Set the number of retransmissions to be carried out before declaring that remote end is not available.
    pub(crate) fn set_keepalive_retries(&mut self, retries: Option<u32>) -> &mut Self {
        self.tcp_keepalive_config.retries = retries;
        self
    }

    /// Set the value of `TCP_NODELAY` option for accepted connections.
    pub(crate) fn set_nodelay(&mut self, enabled: bool) -> &mut Self {
    ...
    ///
    /// Default is `true`.
    pub(crate) fn set_sleep_on_errors(&mut self, val: bool) {
            ...
            match ready!(self.listener.poll_accept(cx)) {
                ...
                Err(e) => {
                    ...
                    if self.sleep_on_errors {
                        ...
                        // Sleep 1s.
                        let mut timeout =
                            Box::pin(crate::custom_tokio::time::sleep(Duration::from_secs(1)));
...
mod addr_stream {
    ...
    use crate::custom_hyper::common::{task, Pin, Poll};

    pin_project_lite::pin_project! {
        /// A transport returned yieled by `AddrIncoming`.
        #[derive(Debug)]
        pub(crate) struct AddrStream {
            #[pin]
            inner: TcpStream,
    ...
    impl AddrStream {
        ...
        pub(crate) fn poll_peek(
            &mut self,
            cx: &mut task::Context<'_>,
            buf: &mut crate::custom_tokio::io::ReadBuf<'_>,
...
#[cfg(test)]
mod tests {
    use crate::custom_hyper::server::tcp::TcpKeepaliveConfig;
    use std::time::Duration;
