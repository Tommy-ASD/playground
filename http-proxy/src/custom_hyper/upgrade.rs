...
pub(crate) struct OnUpgrade {
    rx: Option<oneshot::Receiver<crate::custom_hyper::Result<Upgraded>>>,
}
...
pub(super) struct Pending {
    tx: oneshot::Sender<crate::custom_hyper::Result<Upgraded>>,
}
...
impl Future for OnUpgrade {
    type Output = Result<Upgraded, crate::custom_hyper::Error>;
        ...
        match self.rx {
            Some(ref mut rx) => Pin::new(rx).poll(cx).map(|res| match res {
                ...
                Err(_oneshot_canceled) => {
                    Err(crate::custom_hyper::Error::new_canceled().with(UpgradeExpected))
                }
            }),
            None => Poll::Ready(Err(crate::custom_hyper::Error::new_user_no_upgrade())),
        }
    }
}
...
impl Pending {
    ...
    pub(super) fn manual(self) {
        ...
        let _ = self
            .tx
            .send(Err(crate::custom_hyper::Error::new_user_manual_upgrade()));
    }
}
