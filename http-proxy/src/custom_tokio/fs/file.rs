...
use self::State::*;
use crate::custom_tokio::fs::{asyncify, OpenOptions};
use crate::custom_tokio::io::blocking::Buf;
use crate::custom_tokio::io::{AsyncRead, AsyncSeek, AsyncWrite, ReadBuf};
...
#[cfg(not(test))]
use crate::custom_tokio::blocking::JoinHandle;
#[cfg(not(test))]
use crate::custom_tokio::blocking::{spawn_blocking, spawn_mandatory_blocking};
#[cfg(not(test))]
...
/// [`AsyncSeek`]: trait@crate::custom_tokio::io::AsyncSeek
/// [`flush`]: fn@crate::custom_tokio::io::AsyncWriteExt::flush
/// [`sync_all`]: fn@crate::custom_tokio::fs::File::sync_all
...
/// use crate::custom_tokio::io::AsyncWriteExt; // for write_all()
///
...
/// ```no_run
/// use crate::custom_tokio::fs::File;
/// use crate::custom_tokio::io::AsyncReadExt; // for read_to_end()
///
/// # async fn dox() -> std::io::Result<()> {
...
impl File {
    ...
    ///
    /// [`read_to_end`]: fn@crate::custom_tokio::io::AsyncReadExt::read_to_end
    /// [`AsyncReadExt`]: trait@crate::custom_tokio::io::AsyncReadExt
    pub(crate) async fn open(path: impl AsRef<Path>) -> io::Result<File> {
    ...
    /// [`AsyncWriteExt`]: trait@crate::custom_tokio::io::AsyncWriteExt
    pub(crate) async fn create(path: impl AsRef<Path>) -> io::Result<File> {
        let path = path.as_ref().to_owned();
    ...
    /// Converts a [`std::fs::File`][std] to a [`crate::custom_tokio::fs::File`][file].
    ///
    /// [std]: std::fs::File
    /// [file]: File
    ///
    ...
    /// let file = crate::custom_tokio::fs::File::from_std(std_file);
    /// ```
    ...
    /// # }
    /// ```
    pub(crate) async fn try_clone(&self) -> io::Result<File> {
...
impl fmt::Debug for File {
    fn fmt(&self, fmt: &mut fmt::Formatter<'_>) -> fmt::Result {
        fmt.debug_struct("crate::custom_tokio::fs::File")
            .field("std", &self.std)
            .finish()
    }
}
...
cfg_windows! {
    use crate::custom_tokio::os::windows::io::{AsRawHandle, FromRawHandle, RawHandle, AsHandle, BorrowedHandle};
