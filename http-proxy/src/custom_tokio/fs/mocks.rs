...
mock! {
    #[derive(Debug)]
    pub(crate)File {
        pub(crate)fn create(pb: PathBuf) -> io::Result<Self>;
        ...
        pub(crate)fn inner_flush(&self) -> io::Result<()>;
        pub(crate)fn inner_read(&self, dst: &mut [u8]) -> io::Result<usize>;
        pub(crate)fn inner_seek(&self, pos: SeekFrom) -> io::Result<u64>;
        pub(crate)fn inner_write(&self, src: &[u8]) -> io::Result<usize>;
        pub(crate)fn metadata(&self) -> io::Result<Metadata>;
        pub(crate)fn open(pb: PathBuf) -> io::Result<Self>;
