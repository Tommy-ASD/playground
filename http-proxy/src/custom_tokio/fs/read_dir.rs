...
pub(crate) async fn read_dir(path: impl AsRef<Path>) -> io::Result<ReadDir> {
...
pub(crate) struct ReadDir(State);
...
impl DirEntry {
    ...
    /// # }
    /// ```
    pub(crate) async fn file_type(&self) -> io::Result<FileType> {
        #[cfg(not(any(
            target_os = "solaris",
            target_os = "illumos",
            target_os = "haiku",
    ...
    #[cfg(unix)]
    pub(crate) fn as_inner(&self) -> &std::fs::DirEntry {
        &self.std
    }
}
