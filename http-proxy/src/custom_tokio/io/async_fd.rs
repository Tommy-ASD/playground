use crate::custom_tokio::io::{Interest, Ready};
use crate::custom_tokio::runtime::io::{ReadyEvent, Registration};
use crate::custom_tokio::runtime::scheduler;
...
/// use crate::custom_tokio::io::unix::AsyncFd;
///
...
///     }
///
///     pub(crate)async fn read(&self, out: &mut [u8]) -> io::Result<usize> {
///         loop {
...
/// [`TcpStream::poll_read_ready`]: struct@crate::custom_tokio::net::TcpStream
/// [`AsyncRead`]: trait@crate::custom_tokio::io::AsyncRead
/// [`AsyncWrite`]: trait@crate::custom_tokio::io::AsyncWrite
pub(crate) struct AsyncFd<T: AsRawFd> {
...
pub(crate) struct AsyncFdReadyGuard<'a, T: AsRawFd> {
...
#[must_use = "You must explicitly choose whether to clear the readiness state by calling a method on ReadyGuard"]
pub(crate) struct AsyncFdReadyMutGuard<'a, T: AsRawFd> {
...
impl<T: AsRawFd> AsyncFd<T> {
    ...
    #[inline]
    #[track_caller]
    pub(crate) fn with_interest(inner: T, interest: Interest) -> io::Result<Self>
    where
        T: AsRawFd,
    {
    ...
    pub(crate) fn poll_read_ready<'a>(
        &'a self,
        cx: &mut Context<'_>,
    ...
    /// [`Waker`]: struct@std::task::Waker
    /// [`Waker::wake`]: method@std::task::Waker::wake
    pub(crate) fn poll_read_ready_mut<'a>(
        &'a mut self,
        cx: &mut Context<'_>,
    ...
    /// [`Waker`]: struct@std::task::Waker
    /// [`Waker::wake`]: method@std::task::Waker::wake
    pub(crate) fn poll_write_ready<'a>(
        &'a self,
        cx: &mut Context<'_>,
    ) -> Poll<io::Result<AsyncFdReadyGuard<'a, T>>> {
    ...
    /// [`Waker`]: struct@std::task::Waker
    /// [`Waker::wake`]: method@std::task::Waker::wake
    pub(crate) fn poll_write_ready_mut<'a>(
        &'a mut self,
        cx: &mut Context<'_>,
    ) -> Poll<io::Result<AsyncFdReadyMutGuard<'a, T>>> {
        let event = ready!(self.registration.poll_write_ready(cx))?;
    ...
    /// }
    /// ```
    pub(crate) async fn ready(&self, interest: Interest) -> io::Result<AsyncFdReadyGuard<'_, T>> {
    ...
    pub(crate) async fn readable<'a>(&'a self) -> io::Result<AsyncFdReadyGuard<'a, T>> {
    ...
    /// resource mutably when handling the [`AsyncFdReadyMutGuard`].
    #[allow(clippy::needless_lifetimes)] // The lifetime improves rustdoc rendering.
    pub(crate) async fn readable_mut<'a>(&'a mut self) -> io::Result<AsyncFdReadyMutGuard<'a, T>> {
    ...
    /// [`AsyncFdReadyGuard`].
    #[allow(clippy::needless_lifetimes)] // The lifetime improves rustdoc rendering.
    pub(crate) async fn writable<'a>(&'a self) -> io::Result<AsyncFdReadyGuard<'a, T>> {
    ...
    /// resource mutably when handling the [`AsyncFdReadyMutGuard`].
    #[allow(clippy::needless_lifetimes)] // The lifetime improves rustdoc rendering.
    pub(crate) async fn writable_mut<'a>(&'a mut self) -> io::Result<AsyncFdReadyMutGuard<'a, T>> {
    ...
    ///
    /// ```no_run
    /// use crate::custom_tokio::io::{Interest, unix::AsyncFd};
    ///
    /// use std::io;
    ...
    pub(crate) async fn async_io_mut<R>(
        &mut self,
        interest: Interest,
...
impl<'a, Inner: AsRawFd> AsyncFdReadyGuard<'a, Inner> {
    ...
    /// }
    /// ```
    pub(crate) fn clear_ready_matching(&mut self, ready: Ready) {
    ...
    pub(crate) fn retain_ready(&mut self) {
        // no-op
    }
    ...
    /// the guard.
    ///
    /// [`Ready`]: crate::custom_tokio::io::Ready
    ...
    pub(crate) fn get_ref(&self) -> &'a AsyncFd<Inner> {
        self.async_fd
    }

    /// Returns a shared reference to the backing object of the inner [`AsyncFd`].
    pub(crate) fn get_inner(&self) -> &'a Inner {
...
impl<'a, Inner: AsRawFd> AsyncFdReadyMutGuard<'a, Inner> {
    ...
    /// Returns a shared reference to the inner [`AsyncFd`].
    pub(crate) fn get_ref(&self) -> &AsyncFd<Inner> {
        self.async_fd
    }

    /// Returns a mutable reference to the inner [`AsyncFd`].
    pub(crate) fn get_mut(&mut self) -> &mut AsyncFd<Inner> {
        self.async_fd
    }
    ...
    pub(crate) fn get_inner_mut(&mut self) -> &mut Inner {
