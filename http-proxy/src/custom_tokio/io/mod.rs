...
//! _asynchronous_ &mdash; meaning that reading from or writing to a `crate::custom_tokio::io`
...
//! can do the same with [`crate::custom_tokio::fs::File`][`File`]:
//!
//! ```no_run
//! use crate::custom_tokio::io::{self, AsyncReadExt};
...
//! ```
//!
//! [`File`]: crate::custom_tokio::fs::File
//! [`TcpStream`]: crate::custom_tokio::net::TcpStream
...
//! and therefore, `crate::custom_tokio::io` does as well.
//!
...
//!
//! ```no_run
//! use crate::custom_tokio::io::{self, BufReader, AsyncBufReadExt};
//! use crate::custom_tokio::fs::File;
//!
//! #[crate::custom_tokio::main]
//! async fn main() -> io::Result<()> {
...
//! to [`write`](crate::custom_tokio::io::AsyncWriteExt::write). However, you **must** flush
//! [`BufWriter`] to ensure that any buffered data is written.
//!
//! ```no_run
//! use crate::custom_tokio::io::{self, BufWriter, AsyncWriteExt};
//! use crate::custom_tokio::fs::File;
//!
//! #[crate::custom_tokio::main]
//! async fn main() -> io::Result<()> {
...
//! [`BufReader`]: crate::custom_tokio::io::BufReader
//! [`BufWriter`]: crate::custom_tokio::io::BufWriter
//!
...
mod async_buf_read;
pub(crate) use self::async_buf_read::AsyncBufRead;

mod async_read;
pub(crate) use self::async_read::AsyncRead;

mod async_seek;
pub(crate) use self::async_seek::AsyncSeek;

mod async_write;
pub(crate) use self::async_write::AsyncWrite;

mod read_buf;
pub(crate) use self::read_buf::ReadBuf;

// Re-export some types from `std::io` so that users don't have to deal
// with conflicts when `use`ing `crate::custom_tokio::io` and `std::io`.
#[doc(no_inline)]
pub(crate) use std::io::{Error, ErrorKind, Result, SeekFrom};
...
cfg_aio! {
    ...
    pub(crate)mod bsd {
        ...
        pub(crate)use poll_aio::{Aio, AioEvent, AioSource};
    }
}
...
cfg_net_unix! {
    ...
    pub(crate)mod unix {
        ...
        pub(crate)use super::async_fd::{AsyncFd, AsyncFdReadyGuard, AsyncFdReadyMutGuard, TryIoError};
    }
}
...
cfg_io_std! {
    ...
    mod stderr;
    pub(crate)use stderr::{stderr, Stderr};

    mod stdin;
    pub(crate)use stdin::{stdin, Stdin};

    mod stdout;
    pub(crate)use stdout::{stdout, Stdout};
}

cfg_io_util! {
    mod split;
    pub(crate)use split::{split, ReadHalf, WriteHalf};
...
cfg_io_blocking! {
    ...
    mod sys {
        // TODO: don't rename
        pub(crate) use crate::custom_tokio::blocking::spawn_blocking as run;
        pub(crate) use crate::custom_tokio::blocking::JoinHandle as Blocking;
    }
}
