...
impl<W> crate::custom_tokio::io::AsyncWrite for SplitByUtf8BoundaryIfWindows<W>
where
    W: AsyncWrite + Unpin,
{
    fn poll_write(
        ...
        // to fixup it.
        if cfg!(not(any(target_os = "windows", test))) || buf.len() <= crate::custom_tokio::io::blocking::MAX_BUF
        {
            return call_inner(buf);
        }

        buf = &buf[..crate::custom_tokio::io::blocking::MAX_BUF];
...
#[cfg(not(loom))]
mod tests {
    use crate::custom_tokio::io::blocking::MAX_BUF;
    ...
    impl crate::custom_tokio::io::AsyncWrite for TextMockWriter {
        fn poll_write(
            self: Pin<&mut Self>,
            _cx: &mut Context<'_>,
            buf: &[u8],
    ...
    impl crate::custom_tokio::io::AsyncWrite for LoggingMockWriter {
        fn poll_write(
            mut self: Pin<&mut Self>,
            _cx: &mut Context<'_>,
            buf: &[u8],
