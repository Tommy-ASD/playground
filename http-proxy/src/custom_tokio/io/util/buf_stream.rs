use crate::custom_tokio::io::util::{BufReader, BufWriter};
use crate::custom_tokio::io::{AsyncBufRead, AsyncRead, AsyncSeek, AsyncWrite, ReadBuf};
...
impl<RW: AsyncRead + AsyncWrite> BufStream<RW> {
    ...
    pub(crate)fn new(stream: RW) -> BufStream<RW> {
        BufStream {
    ...
    ///
    /// It is inadvisable to directly read from the underlying I/O object.
    pub(crate)fn get_mut(&mut self) -> &mut RW {
    ...
    ///
    /// It is inadvisable to directly read from the underlying I/O object.
    pub(crate)fn get_pin_mut(self: Pin<&mut Self>) -> Pin<&mut RW> {
