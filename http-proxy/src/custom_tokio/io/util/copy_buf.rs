use crate::custom_tokio::io::{AsyncBufRead, AsyncWrite};
...
cfg_io_util! {
    ...
    /// `writer` is returned.
    ///
    /// This is a [`crate::custom_tokio::io::copy`] alternative for [`AsyncBufRead`] readers
    /// with no extra buffer allocation, since [`AsyncBufRead`] allow access
    /// to the reader's inner buffer.
    ///
    /// [`crate::custom_tokio::io::copy`]: crate::custom_tokio::io::copy
    ...
    /// # }
    /// ```
    pub(crate)async fn copy_buf<'a, R, W>(reader: &'a mut R, writer: &'a mut W) -> io::Result<u64>
    where
