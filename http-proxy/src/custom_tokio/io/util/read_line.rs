...
pin_project! {
    /// Future for the [`read_line`](crate::custom_tokio::io::AsyncBufReadExt::read_line) method.
    #[derive(Debug)]
    #[must_use = "futures do nothing unless you `.await` or poll them"]
    pub(crate)struct ReadLine<'a, R: ?Sized> {
        reader: &'a mut R,
...
pub(crate) fn finish_string_read(
...
pub(crate) fn read_line_internal<R: AsyncBufRead + ?Sized>(
    reader: Pin<&mut R>,
    cx: &mut Context<'_>,
    output: &mut String,
    buf: &mut Vec<u8>,
