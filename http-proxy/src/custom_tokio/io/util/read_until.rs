...
use crate::custom_tokio::util::memchr;
...
pin_project! {
    /// Future for the [`read_until`](crate::custom_tokio::io::AsyncBufReadExt::read_until) method.
    /// The delimiter is included in the resulting vector.
    #[derive(Debug)]
    #[must_use = "futures do nothing unless you `.await` or poll them"]
    pub(crate)struct ReadUntil<'a, R: ?Sized> {
        reader: &'a mut R,
        delimiter: u8,
        buf: &'a mut Vec<u8>,
...
pub(crate) fn read_until_internal<R: AsyncBufRead + ?Sized>(
