...
/// async fn main() {
///     let res = crate::custom_tokio::try_join!(
///         do_stuff_async(),
///         more_async_work());
///
///     match res {
...
/// #[crate::custom_tokio::main]
/// async fn main() {
///     let handle1 = crate::custom_tokio::spawn(do_stuff_async());
///     let handle2 = crate::custom_tokio::spawn(more_async_work());
///     match crate::custom_tokio::try_join!(flatten(handle1), flatten(handle2)) {
///         Ok(val) => {
///             // do something with the values
///         }
///         Err(err) => {
...
macro_rules! try_join {
    (@ {
        ...
        use $crate::custom_tokio::macros::support::{maybe_done, poll_fn, Future, Pin};
    ...
    (@ { ( $($s:tt)* ) ( $($n:tt)* ) $($t:tt)* } $e:expr, $($r:tt)* ) => {
      $crate::custom_tokio::try_join!(@{ ($($s)* _) ($($n)* + 1) $($t)* ($($s)*) $e, } $($r)*)
    };

    // ===== Entry point =====

    ( $($e:expr),+ $(,)?) => {
        $crate::custom_tokio::try_join!(@{ () (0) } $($e,)*)
    };
