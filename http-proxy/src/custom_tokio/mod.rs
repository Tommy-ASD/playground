...
//! [sync]: crate::custom_tokio::sync
//! [time]: crate::custom_tokio::time
//! [io]: #asynchronous-io
//! [net]: crate::custom_tokio::net
//! [fs]: crate::custom_tokio::fs
//! [process]: crate::custom_tokio::process
//! [signal]: crate::custom_tokio::signal
//! [fs]: crate::custom_tokio::fs
//! [runtime]: crate::custom_tokio::runtime
...
//! needs to `crate::custom_tokio::spawn` and use a `TcpStream`.
//!
//! ```toml
...
//! units of execution called [_tasks_][tasks]. The [`crate::custom_tokio::task`] module provides
...
//!   task context.
//!
//! The [`crate::custom_tokio::task`] module is present only when the "rt" feature flag
//! is enabled.
//!
//! [tasks]: task/index.html#what-are-tasks
//! [`crate::custom_tokio::task`]: crate::custom_tokio::task
//! [`spawn`]: crate::custom_tokio::task::spawn()
//! [`JoinHandle`]: crate::custom_tokio::task::JoinHandle
//! [blocking]: task/index.html#blocking-and-yielding
//!
//! The [`crate::custom_tokio::sync`] module contains synchronization primitives to use when
...
//!
//! The `crate::custom_tokio::sync` module is present only when the "sync" feature flag is
//! enabled.
//!
//! [`crate::custom_tokio::sync`]: crate::custom_tokio::sync
//! [`Mutex`]: crate::custom_tokio::sync::Mutex
//! [`Barrier`]: crate::custom_tokio::sync::Barrier
//! [`oneshot`]: crate::custom_tokio::sync::oneshot
//! [`mpsc`]: crate::custom_tokio::sync::mpsc
//! [`watch`]: crate::custom_tokio::sync::watch
//! [`broadcast`]: crate::custom_tokio::sync::broadcast
//!
//! The [`crate::custom_tokio::time`] module provides utilities for tracking time and
//! scheduling work. This includes functions for setting [timeouts][timeout] for
//! tasks, [sleeping][sleep] work to run in the future, or [repeating an operation at an
//! interval][interval].
//!
//! In order to use `crate::custom_tokio::time`, the "time" feature flag must be enabled.
//!
//! [`crate::custom_tokio::time`]: crate::custom_tokio::time
//! [sleep]: crate::custom_tokio::time::sleep()
//! [interval]: crate::custom_tokio::time::interval()
//! [timeout]: crate::custom_tokio::time::timeout()
//!
//! Finally, Tokio provides a _runtime_ for executing asynchronous tasks. Most
//! applications can use the [`#[crate::custom_tokio::main]`][main] macro to run their code on the
//! Tokio runtime. However, this macro provides only basic configuration options. As
//! an alternative, the [`crate::custom_tokio::runtime`] module provides more powerful APIs for configuring
//! and managing runtimes. You should use that module if the `#[crate::custom_tokio::main]` macro doesn't
...
//! flag enables the `#[crate::custom_tokio::main]` and `#[tokio::test]` attributes.
//!
//! [main]: attr.main.html
//! [`crate::custom_tokio::runtime`]: crate::custom_tokio::runtime
//! [`Builder`]: crate::custom_tokio::runtime::Builder
//! [`Runtime`]: crate::custom_tokio::runtime::Runtime
...
//! [`thread_keep_alive`]: crate::custom_tokio::runtime::Builder::thread_keep_alive()
//!
//! ```
...
//!
//!     let blocking_task = crate::custom_tokio::task::spawn_blocking(|| {
...
//!
//! The [`crate::custom_tokio::io`] module provides Tokio's asynchronous core I/O primitives,
...
//!
//! * [`crate::custom_tokio::net`], which contains non-blocking versions of [TCP], [UDP], and
//!   [Unix Domain Sockets][UDS] (enabled by the "net" feature flag),
//! * [`crate::custom_tokio::fs`], similar to [`std::fs`] but for performing filesystem I/O
//!   asynchronously (enabled by the "fs" feature flag),
//! * [`crate::custom_tokio::signal`], for asynchronously handling Unix and Windows OS signals
//!   (enabled by the "signal" feature flag),
//! * [`crate::custom_tokio::process`], for spawning and managing child processes (enabled by
//!   the "process" feature flag).
//!
//! [`crate::custom_tokio::io`]: crate::custom_tokio::io
//! [`AsyncRead`]: crate::custom_tokio::io::AsyncRead
//! [`AsyncWrite`]: crate::custom_tokio::io::AsyncWrite
//! [`AsyncBufRead`]: crate::custom_tokio::io::AsyncBufRead
//! [`std::io`]: std::io
//! [`crate::custom_tokio::net`]: crate::custom_tokio::net
//! [TCP]: crate::custom_tokio::net::tcp
//! [UDP]: crate::custom_tokio::net::UdpSocket
//! [UDS]: crate::custom_tokio::net::unix
//! [`crate::custom_tokio::fs`]: crate::custom_tokio::fs
//! [`std::fs`]: std::fs
//! [`crate::custom_tokio::signal`]: crate::custom_tokio::signal
//! [`crate::custom_tokio::process`]: crate::custom_tokio::process
//!
//! # Examples
//!
...
//! - `rt`: Enables `crate::custom_tokio::spawn`, the current-thread scheduler,
//!         and non-scheduler utilities.
//! - `rt-multi-thread`: Enables the heavier, multi-threaded, work-stealing scheduler.
//! - `io-util`: Enables the IO based `Ext` traits.
//! - `io-std`: Enable `Stdout`, `Stdin` and `Stderr` types.
//! - `net`: Enables `crate::custom_tokio::net` types such as `TcpStream`, `UnixStream` and
//!          `UdpSocket`, as well as (on Unix-like systems) `AsyncFd` and (on
//!          FreeBSD) `PollAio`.
//! - `time`: Enables `crate::custom_tokio::time` types and allows the schedulers to enable
//!           the built in timer.
//! - `process`: Enables `crate::custom_tokio::process` types.
//! - `macros`: Enables `#[crate::custom_tokio::main]` and `#[tokio::test]` macros.
//! - `sync`: Enables all `crate::custom_tokio::sync` types.
//! - `signal`: Enables all `crate::custom_tokio::signal` types.
//! - `fs`: Enables `crate::custom_tokio::fs` types.
...
//!
//! Using this flag enables the use of `crate::custom_tokio::net` on the wasm32-wasi target.
...
pub(crate) mod io;
pub(crate) mod net;
pub(crate) use tokio_macros::main;
...
mod trace {
    ...
    cfg_taskdump! {
        pub(crate) use crate::custom_tokio::runtime::task::trace::trace_leaf;
    }
...
///
/// ```rust,no_run
/// use crate::custom_tokio::sync::mpsc;
///
...
cfg_macros! {
    ...
    /// change.
    #[doc(hidden)]
    pub(crate)use tokio_macros::select_priv_declare_output_enum;

    /// Implementation detail of the `select!` macro. This macro is **not**
    /// intended to be used as part of the public API and is permitted to
    /// change.
    #[doc(hidden)]
    pub(crate)use tokio_macros::select_priv_clean_pattern;
    ...
    cfg_rt! {
        ...
        #[doc(inline)]
        pub(crate)use tokio_macros::test;

        cfg_not_rt_multi_thread! {
            #[cfg(not(test))] // Work around for rust-lang/rust#62127
            #[doc(inline)]
            pub(crate)use tokio_macros::main_rt as main;

            #[doc(inline)]
            pub(crate)use tokio_macros::test_rt as test;
        }
    }
    ...
    cfg_not_rt! {
        #[cfg(not(test))]
        #[doc(inline)]
        pub(crate)use tokio_macros::main_fail as main;

        #[doc(inline)]
        pub(crate)use tokio_macros::test_fail as test;
    }
}
