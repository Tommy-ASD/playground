use crate::custom_tokio::io::{Interest, PollEvented, ReadBuf, Ready};
use crate::custom_tokio::net::{to_socket_addrs, ToSocketAddrs};
...
cfg_net! {
    ...
    /// ```no_run
    /// use crate::custom_tokio::net::UdpSocket;
    /// use std::io;
    ///
    ...
    ///
    /// ```no_run
    /// use crate::custom_tokio::{net::UdpSocket, sync::mpsc};
...
impl UdpSocket {
    ...
    /// }
    /// ```
    pub(crate) async fn bind<A: ToSocketAddrs>(addr: A) -> io::Result<UdpSocket> {
    ...
    /// explicitly with [`Runtime::enter`](crate::custom_tokio::runtime::Runtime::enter) function.
    ///
    /// # Example
    ///
    /// ```no_run
    ...
    /// ```
    #[track_caller]
    pub(crate) fn from_std(socket: net::UdpSocket) -> io::Result<UdpSocket> {
        let io = mio::net::UdpSocket::from_std(socket);
        UdpSocket::new(io)
    }

    /// Turns a [`crate::custom_tokio::net::UdpSocket`] into a [`std::net::UdpSocket`].
    ///
    ...
    ///     let tokio_socket = crate::custom_tokio::net::UdpSocket::bind("127.0.0.1:0").await?;
    ...
    /// ```
    ///
    /// [`crate::custom_tokio::net::UdpSocket`]: UdpSocket
    /// [`std::net::UdpSocket`]: std::net::UdpSocket
    /// [`set_nonblocking`]: fn@std::net::UdpSocket::set_nonblocking
    pub(crate) fn into_std(self) -> io::Result<std::net::UdpSocket> {
        #[cfg(unix)]
        {
    ...
    /// # }
    /// ```
    pub(crate) async fn connect<A: ToSocketAddrs>(&self, addr: A) -> io::Result<()> {
        let addrs = to_socket_addrs(addr).await?;
        let mut last_err = None;
    ...
    ///
    /// ```no_run
    /// use crate::custom_tokio::io::{self, Interest};
    /// use crate::custom_tokio::net::UdpSocket;
    ///
    /// #[crate::custom_tokio::main]
    /// async fn main() -> io::Result<()> {
    ...
    pub(crate) fn poll_recv_ready(&self, cx: &mut Context<'_>) -> Poll<io::Result<()>> {
    ...
    cfg_io_util! {
        ...
        /// }
        /// ```
        pub(crate)fn try_recv_buf<B: BufMut>(&self, buf: &mut B) -> io::Result<usize> {
        ...
        /// }
        /// ```
        pub(crate)async fn recv_buf<B: BufMut>(&self, buf: &mut B) -> io::Result<usize> {
        ...
        /// }
        /// ```
        pub(crate)fn try_recv_buf_from<B: BufMut>(&self, buf: &mut B) -> io::Result<(usize, SocketAddr)> {
            self.io.registration().try_io(Interest::READABLE, || {
                let dst = buf.chunk_mut();
                let dst =
                    unsafe { &mut *(dst as *mut _ as *mut [std::mem::MaybeUninit<u8>] as *mut [u8]) };
        ...
        /// }
        /// ```
        pub(crate)async fn recv_buf_from<B: BufMut>(&self, buf: &mut B) -> io::Result<(usize, SocketAddr)> {
            self.io.registration().async_io(Interest::READABLE, || {
                let dst = buf.chunk_mut();
                let dst =
                    unsafe { &mut *(dst as *mut _ as *mut [std::mem::MaybeUninit<u8>] as *mut [u8]) };
    ...
    ///
    /// [`ToSocketAddrs`]: crate::custom_tokio::net::ToSocketAddrs
    ///
    /// # Cancel safety
    ///
    ...
    /// }
    /// ```
    pub(crate) async fn send_to<A: ToSocketAddrs>(
        &self,
        buf: &[u8],
        target: A,
    ) -> io::Result<usize> {
    ...
    /// }
    /// ```
    pub(crate) fn try_recv_from(&self, buf: &mut [u8]) -> io::Result<(usize, SocketAddr)> {
        self.io
            .registration()
    ...
    /// [packet injection attack]: https://en.wikipedia.org/wiki/Packet_injection
    pub(crate) async fn peek_from(&self, buf: &mut [u8]) -> io::Result<(usize, SocketAddr)> {
        self.io
            .registration()
    ...
    /// [packet injection attack]: https://en.wikipedia.org/wiki/Packet_injection
    pub(crate) fn try_peek_from(&self, buf: &mut [u8]) -> io::Result<(usize, SocketAddr)> {
        self.io
            .registration()
    ...
    /// [packet injection attack]: https://en.wikipedia.org/wiki/Packet_injection
    pub(crate) fn poll_peek_sender(&self, cx: &mut Context<'_>) -> Poll<io::Result<SocketAddr>> {
        self.io
            .registration()
    ...
    ///
    /// [packet injection attack]: https://en.wikipedia.org/wiki/Packet_injection
    pub(crate) fn try_peek_sender(&self) -> io::Result<SocketAddr> {
        self.io
            .registration()
    ...
    pub(crate) fn set_tos(&self, tos: u32) -> io::Result<()> {
    ...
    pub(crate) fn device(&self) -> io::Result<Option<Vec<u8>>> {
...
cfg_windows! {
    use crate::custom_tokio::os::windows::io::{AsRawSocket, RawSocket};
    use crate::custom_tokio::os::windows::io::{AsSocket, BorrowedSocket};
