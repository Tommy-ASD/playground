...
use crate::custom_tokio::net::unix::SocketAddr;
...
cfg_net_unix! {
    ...
    /// use crate::custom_tokio::net::UnixDatagram;
...
impl UnixDatagram {
    ...
    /// # }
    /// ```
    pub(crate) fn bind<P>(path: P) -> io::Result<UnixDatagram>
    where
        P: AsRef<Path>,
    {
    ...
    /// ```
    #[track_caller]
    pub(crate) fn from_std(datagram: net::UnixDatagram) -> io::Result<UnixDatagram> {
        let socket = mio::net::UnixDatagram::from_std(datagram);
        let io = PollEvented::new(socket)?;
        Ok(UnixDatagram { io })
    }

    /// Turns a [`crate::custom_tokio::net::UnixDatagram`] into a [`std::os::unix::net::UnixDatagram`].
    ///
    ...
    /// let tokio_socket = crate::custom_tokio::net::UnixDatagram::bind("/path/to/the/socket")?;
    ...
    /// }
    /// ```
    pub(crate) fn try_send_to<P>(&self, buf: &[u8], target: P) -> io::Result<usize>
    where
        P: AsRef<Path>,
    {
        self.io
    ...
    /// # }
    /// ```
    pub(crate) async fn send_to<P>(&self, buf: &[u8], target: P) -> io::Result<usize>
    where
        P: AsRef<Path>,
    {
        self.io
