...
use crate::custom_tokio::net::unix::{SocketAddr, UnixStream};
...
impl UnixListener {
    ...
    #[track_caller]
    pub(crate) fn bind<P>(path: P) -> io::Result<UnixListener>
    where
        P: AsRef<Path>,
    {
    ...
    /// explicitly with [`Runtime::enter`](crate::custom_tokio::runtime::Runtime::enter) function.
    #[track_caller]
    pub(crate) fn from_std(listener: net::UnixListener) -> io::Result<UnixListener> {
        let listener = mio::net::UnixListener::from_std(listener);
        let io = PollEvented::new(listener)?;
        Ok(UnixListener { io })
    }

    /// Turns a [`crate::custom_tokio::net::UnixListener`] into a [`std::os::unix::net::UnixListener`].
    ///
    ...
    /// # use std::error::Error;
    /// # async fn dox() -> Result<(), Box<dyn Error>> {
    /// let tokio_listener = crate::custom_tokio::net::UnixListener::bind("/path/to/the/socket")?;
    ...
    pub(crate) async fn accept(&self) -> io::Result<(UnixStream, SocketAddr)> {
        let (mio, addr) = self
            .io
            .registration()
