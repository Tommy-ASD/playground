...
//!
//! ```no_run
//! use crate::custom_tokio::process::Command;
//!
...
//!
//! ```no_run
//! use crate::custom_tokio::io::{BufReader, AsyncBufReadExt};
//! use crate::custom_tokio::process::Command;
//!
...
//!
//! [`Command`]: crate::custom_tokio::process::Command
//! [`Command::kill_on_drop`]: crate::custom_tokio::process::Command::kill_on_drop
//! [`Child`]: crate::custom_tokio::process::Child
...
use crate::custom_tokio::process::kill::Kill;
...
cfg_windows! {
    use crate::custom_tokio::os::windows::io::{AsRawHandle, RawHandle};
}
...
impl Command {
    ...
    /// library is expected.
    pub(crate) fn as_std(&self) -> &StdCommand {
        &self.std
    }
    ...
    ///
    /// ```no_run
    /// let mut command = crate::custom_tokio::process::Command::new("sh");
    ...
    cfg_windows! {
        ...
        pub(crate)fn raw_arg<S: AsRef<OsStr>>(&mut self, text_to_append_as_is: S) -> &mut Command {
    ...
    /// # }
    /// ```
    pub(crate) fn env<K, V>(&mut self, key: K, val: V) -> &mut Command
    where
        K: AsRef<OsStr>,
        V: AsRef<OsStr>,
    {
    ...
    /// # }
    /// ```
    pub(crate) fn envs<I, K, V>(&mut self, vars: I) -> &mut Command
    where
    ...
    /// # }
    /// ```
    pub(crate) fn env_remove<K: AsRef<OsStr>>(&mut self, key: K) -> &mut Command {
    ...
    ///
    /// [`canonicalize`]: crate::custom_tokio::fs::canonicalize()
    ///
    /// # Examples
    ///
    /// Basic usage:
    ...
    /// # }
    /// ```
    pub(crate) fn stderr<T: Into<Stdio>>(&mut self, cfg: T) -> &mut Command {
        self.std.stderr(cfg);
        self
    }
    ...
    pub(crate) fn kill_on_drop(&mut self, kill_on_drop: bool) -> &mut Command {
    ...
    cfg_windows! {
        ...
        pub(crate)fn creation_flags(&mut self, flags: u32) -> &mut Command {
    ...
    pub(crate) fn uid(&mut self, id: u32) -> &mut Command {
        self.std.uid(id);
        self
    }
    ...
    #[cfg(unix)]
    #[cfg_attr(docsrs, doc(cfg(unix)))]
    pub(crate) fn gid(&mut self, id: u32) -> &mut Command {
        self.std.gid(id);
        self
    }
    ...
    #[cfg(unix)]
    #[cfg_attr(docsrs, doc(cfg(unix)))]
    pub(crate) fn arg0<S>(&mut self, arg: S) -> &mut Command
    where
        S: AsRef<OsStr>,
    {
        self.std.arg0(arg);
    ...
    #[cfg(unix)]
    #[cfg_attr(docsrs, doc(cfg(unix)))]
    pub(crate) unsafe fn pre_exec<F>(&mut self, f: F) -> &mut Command
    where
    ...
    /// [`crate::custom_tokio::process::Command`] using the `From` trait.
    ///
    /// [unstable]: crate#unstable-features
    /// [`crate::custom_tokio::process::Command`]: crate::custom_tokio::process::Command
    ///
    /// ```no_run
    /// # async fn test() { // allow using await
    /// use crate::custom_tokio::process::Command;
    ...
    pub(crate) fn process_group(&mut self, pgroup: i32) -> &mut Command {
    ...
    /// }
    /// ```
    pub(crate) fn status(&mut self) -> impl Future<Output = io::Result<ExitStatus>> {
    ...
    /// }
    /// ```
    pub(crate) fn output(&mut self) -> impl Future<Output = io::Result<Output>> {
    ...
    F: Future<Output = Result<T, E>> + Kill + Unpin,
{
    ...
    fn poll(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
        ...
        let coop = ready!(crate::custom_tokio::runtime::coop::poll_proceed(cx));
...
pub(crate) struct Child {
    ...
    ///
    /// ```no_run
    /// # let mut child = crate::custom_tokio::process::Command::new("echo").spawn().unwrap();
...
impl Child {
    ...
    /// ```no_run
    /// use crate::custom_tokio::process::Command;
    /// use crate::custom_tokio::sync::oneshot::channel;
    ///
    /// #[crate::custom_tokio::main]
    /// async fn main() {
    ///     let (send, recv) = channel::<()>();
    ///     let mut child = Command::new("sleep").arg("1").spawn().unwrap();
    ///     crate::custom_tokio::spawn(async move { send.send(()) });
    ...
    pub(crate) async fn wait_with_output(mut self) -> io::Result<Output> {
        use crate::custom_tokio::future::try_join3;

        async fn read_to_end<A: AsyncRead + Unpin>(io: &mut Option<A>) -> io::Result<Vec<u8>> {
            let mut vec = Vec::new();
            if let Some(io) = io.as_mut() {
                crate::custom_tokio::io::util::read_to_end(io, &mut vec).await?;
            }
            Ok(vec)
        }
    ...
    macro_rules! impl_traits {
        ($type:ty) => {
            impl $type {
                ...
                pub(crate) fn into_owned_handle(self) -> io::Result<OwnedHandle> {
