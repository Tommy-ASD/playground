...
use crate::custom_tokio::runtime::{scheduler, TryCurrentError};
use crate::custom_tokio::util::markers::SyncNotSend;
...
///
/// [`Handle`]: crate::custom_tokio::runtime::scheduler::Handle
...
impl Context {
    pub(crate) fn set_current(&self, handle: &scheduler::Handle) -> SetCurrentGuard {
...
impl HandleCell {
    pub(crate) const fn new() -> HandleCell {
        HandleCell {
                ...
                if !std::thread::panicking() {
                    panic!(
                        "`EnterGuard` values dropped out of order. Guards returned by \
                         `crate::custom_tokio::runtime::Handle::enter()` must be dropped in the reverse \
