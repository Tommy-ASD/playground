...
use crate::custom_tokio::runtime::io::ScheduledIo;
...
impl RegistrationSet {
    ...
    pub(crate) fn is_shutdown(&self, synced: &Synced) -> bool {
        synced.is_shutdown
    }
    ...
    pub(crate) fn shutdown(&self, synced: &mut Synced) -> Vec<Arc<ScheduledIo>> {
        if synced.is_shutdown {
            return vec![];
        }
