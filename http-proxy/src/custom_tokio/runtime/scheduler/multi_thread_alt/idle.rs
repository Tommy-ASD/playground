...
use crate::custom_tokio::loom::sync::atomic::{AtomicBool, AtomicUsize};
use crate::custom_tokio::loom::sync::MutexGuard;
use crate::custom_tokio::runtime::scheduler::multi_thread_alt::{worker, Core, Handle, Shared};
...
impl Idle {
    pub(crate) fn new(cores: Vec<Box<Core>>, num_workers: usize) -> (Idle, Synced) {
        let idle = Idle {
    ...
    pub(crate) fn snapshot(&self, snapshot: &mut Snapshot) {
        snapshot.update(&self.idle_map)
    }

    /// Try to acquire an available core
    pub(crate) fn try_acquire_available_core(&self, synced: &mut Synced) -> Option<Box<Core>> {
    ...
    pub(crate) fn notify_local(&self, shared: &Shared) {
    ...
    pub(crate) fn notify_remote(&self, synced: MutexGuard<'_, worker::Synced>, shared: &Shared) {
    ...
    pub(crate) fn shutdown(&self, synced: &mut worker::Synced, shared: &Shared) {
    ...
    pub(crate) fn shutdown_unassigned_cores(&self, handle: &Handle, shared: &Shared) {
    ...
    /// that are waiting.
    pub(crate) fn release_core(&self, synced: &mut worker::Synced, core: Box<Core>) {
    ...
    pub(crate) fn try_transition_worker_to_searching(&self, core: &mut Core) {
        debug_assert!(!core.is_searching);
    ...
    pub(crate) fn transition_worker_to_searching_if_needed(
        &self,
        _synced: &mut Synced,
        core: &mut Core,
    ) -> bool {
    ...
    pub(crate) fn transition_worker_to_searching(&self, core: &mut Core) {
        core.is_searching = true;
        self.num_searching.fetch_add(1, AcqRel);
