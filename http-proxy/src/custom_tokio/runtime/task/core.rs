...
use crate::custom_tokio::future::Future;
use crate::custom_tokio::loom::cell::UnsafeCell;
use crate::custom_tokio::runtime::context;
use crate::custom_tokio::runtime::task::raw::{self, Vtable};
use crate::custom_tokio::runtime::task::state::State;
use crate::custom_tokio::runtime::task::{Id, Schedule};
use crate::custom_tokio::util::linked_list;
...
pub(crate) struct Header {
    ...
    pub(crate) owner_id: UnsafeCell<Option<NonZeroU64>>,

    /// The tracing ID for this instrumented task.
    #[cfg(all(tokio_unstable, feature = "tracing"))]
    pub(crate) tracing_id: Option<tracing::Id>,
}
...
pub(crate) struct Trailer {
    ...
    pub(crate) owned: linked_list::Pointers<Header>,
...
generate_addr_of_methods! {
    impl<> Trailer {
        pub(crate) unsafe fn addr_of_owned(self: NonNull<Self>) -> NonNull<linked_list::Pointers<Header>> {
            &self.owned
        }
    }
}
...
impl<T: Future, S: Schedule> Cell<T, S> {
    ...
    /// structures.
    pub(crate) fn new(future: T, scheduler: S, state: State, task_id: Id) -> Box<Cell<T, S>> {
...
impl<T: Future, S: Schedule> Core<T, S> {
    ...
    ///
    /// The caller must ensure it is safe to mutate the `stage` field.
    pub(crate) fn store_output(&self, output: super::Result<T::Output>) {
        // Safety: the caller ensures mutual exclusion to the field.
        unsafe {
    ...
    ///
    /// The caller must ensure it is safe to mutate the `stage` field.
    pub(crate) fn take_output(&self) -> super::Result<T::Output> {
        use std::mem;
...
impl Header {
    ...
    pub(crate) unsafe fn set_owner_id(&self, owner: NonZeroU64) {
        self.owner_id.with_mut(|ptr| *ptr = Some(owner));
    }

    pub(crate) fn get_owner_id(&self) -> Option<NonZeroU64> {
    ...
    pub(crate) unsafe fn get_trailer(me: NonNull<Header>) -> NonNull<Trailer> {
    ...
    /// task.
    pub(crate) unsafe fn get_scheduler<S>(me: NonNull<Header>) -> NonNull<S> {
    ...
    /// The provided raw pointer must point at the header of a task.
    #[cfg(all(tokio_unstable, feature = "tracing"))]
    pub(crate) unsafe fn get_tracing_id(me: &NonNull<Header>) -> Option<&tracing::Id> {
