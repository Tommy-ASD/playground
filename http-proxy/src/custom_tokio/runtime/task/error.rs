...
use super::Id;
use crate::custom_tokio::util::SyncWrapper;
cfg_rt! {
...
impl JoinError {
    ...
    /// async fn main() {
    ///     let err = crate::custom_tokio::spawn(async {
    ///         panic!("boom");
    ///     }).await.unwrap_err();
    ///
    ///     assert!(err.is_panic());
    ...
    /// }
    /// ```
    pub(crate) fn try_into_panic(self) -> Result<Box<dyn Any + Send + 'static>, JoinError> {
        match self.repr {
