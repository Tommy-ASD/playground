use crate::custom_tokio::runtime::task::{Header, RawTask};
...
cfg_rt! {
    ...
    /// in a `crate::custom_tokio::select!` statement and some other branch completes first,
    ...
    ///     let join_handle: task::JoinHandle<Result<i32, io::Error>> = crate::custom_tokio::spawn(async {
    ///         Ok(5 + 3)
    ///     });
    ///
...
impl<T> JoinHandle<T> {
    ...
    ///
    /// handles.push(crate::custom_tokio::spawn(async {
    ...
    /// # #[crate::custom_tokio::main(flavor = "current_thread", start_paused = true)]
    /// # async fn main() {
    /// let handle1 = crate::custom_tokio::spawn(async {
    ///     // do some stuff here
    /// });
    /// let handle2 = crate::custom_tokio::spawn(async {
    ...
    ///
    /// ```rust
    /// use crate::custom_tokio::{time, task};
    ///
    /// # #[crate::custom_tokio::main(flavor = "current_thread", start_paused = true)]
    /// # async fn main() {
    /// let mut handles = Vec::new();
