...
use crate::custom_tokio::runtime::task::core::{Core, Trailer};
use crate::custom_tokio::runtime::task::{Cell, Harness, Header, Id, Schedule, State};
...
pub(crate) struct Vtable {
    ...
    pub(crate) drop_abort_handle: unsafe fn(NonNull<Header>),
    ...
    pub(crate) scheduler_offset: usize,
...
impl RawTask {
    pub(crate) fn new<T, S>(task: T, scheduler: S, id: Id) -> RawTask
    where
        T: Future,
        S: Schedule,
    {
    ...
    pub(crate) fn trailer_ptr(&self) -> NonNull<Trailer> {
...
unsafe fn schedule<S: Schedule>(ptr: NonNull<Header>) {
    use crate::custom_tokio::runtime::task::{Notified, Task};
