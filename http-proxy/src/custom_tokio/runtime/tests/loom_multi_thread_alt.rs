...
use crate::custom_tokio::runtime::tests::loom_oneshot as oneshot;
use crate::custom_tokio::runtime::{self, Runtime};
use crate::custom_tokio::{spawn, task};
...
mod group_a {
    ...
    fn racy_shutdown() {
        loom::model(|| {
            ...
            pool.spawn(track(async {
                crate::custom_tokio::task::block_in_place(|| {});
            }));
            pool.spawn(track(async {}));
            drop(pool);
        });
    ...
    fn only_blocking_inner(first_pending: bool) {
        loom::model(move || {
            ...
            pool.spawn(track(async move {
                crate::custom_tokio::task::block_in_place(move || {
                    block_tx.send(());
                });
                if first_pending {
...
mod group_b {
    ...
    fn join_output() {
        loom::model(|| {
            ...
            rt.block_on(async {
                let t = crate::custom_tokio::spawn(track(async { "hello" }));
    ...
    fn poll_drop_handle_then_drop() {
        loom::model(|| {
            ...
            rt.block_on(async move {
                let mut t = crate::custom_tokio::spawn(track(async { "hello" }));
    ...
    fn complete_block_on_under_load() {
        loom::model(|| {
            ...
            pool.block_on(async {
                ...
                crate::custom_tokio::spawn(track(async {
                    for _ in 0..2 {
                        task::yield_now().await;
                    }
                }));
    ...
    fn pool_concurrent_park_with_steal_with_inject() {
        ...
        model.check(|| {
            ...
            pool.spawn(track(async move {
                ...
                crate::custom_tokio::spawn(task(1, flag.clone()));
                crate::custom_tokio::spawn(task(2, flag.clone()));
                crate::custom_tokio::spawn(async move {
