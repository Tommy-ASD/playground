...
use crate::custom_tokio::runtime::driver::{self, IoHandle, IoStack};
use crate::custom_tokio::time::error::Error;
use crate::custom_tokio::time::{Clock, Duration};
...
pub(crate)struct Inner {
    ...
    pub(crate) state: Mutex<InnerState>,
...
impl Handle {
    ...
    pub(self) unsafe fn reregister(
        ...
        let waker = unsafe {
            ...
            if self.is_shutdown() {
                unsafe { entry.fire(Err(crate::custom_tokio::time::error::Error::shutdown())) }
            } else {
                ...
                match unsafe { lock.wheel.insert(entry) } {
                    ...
                    Err((entry, crate::custom_tokio::time::error::InsertError::Elapsed)) => unsafe {
                        entry.fire(Ok(()))
                    },
                }
            }
...
impl Inner {
    ...
    pub(crate) fn lock(&self) -> crate::custom_tokio::loom::sync::MutexGuard<'_, InnerState> {
        self.state.lock()
    }
