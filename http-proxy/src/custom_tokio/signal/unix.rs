...
use crate::custom_tokio::runtime::signal::Handle;
use crate::custom_tokio::signal::registry::{globals, EventId, EventInfo, Globals, Init, Storage};
...
pub(crate) struct SignalKind(libc::c_int);
...
impl SignalKind {
    ...
    /// signals.
    /// ```rust,no_run
    /// # use crate::custom_tokio::signal::unix::SignalKind;
    /// # let signum = -1;
    ...
    pub(crate) const fn from_raw(signum: std::os::raw::c_int) -> Self {
    ...
    /// ```
    pub(crate) const fn as_raw_value(&self) -> std::os::raw::c_int {
        self.0
    }
    ...
    /// By default, the process is terminated by this signal.
    pub(crate) const fn hangup() -> Self {
        Self(libc::SIGHUP)
    }
    ...
    /// By default, the process is terminated by this signal.
    pub(crate) const fn interrupt() -> Self {
        Self(libc::SIGINT)
    }
    ...
    pub(crate) const fn terminate() -> Self {
        Self(libc::SIGTERM)
    }
    ...
    /// By default, the process is terminated by this signal.
    pub(crate) const fn user_defined1() -> Self {
        Self(libc::SIGUSR1)
    }
    ...
    /// On Unix systems this is a user defined signal.
    /// By default, the process is terminated by this signal.
    pub(crate) const fn user_defined2() -> Self {
        Self(libc::SIGUSR2)
    }
    ...
    pub(crate) const fn window_change() -> Self {
        Self(libc::SIGWINCH)
    }
}
...
///
/// ```rust,no_run
/// use crate::custom_tokio::signal::unix::{signal, SignalKind};
///
...
#[track_caller]
pub(crate) fn signal(kind: SignalKind) -> io::Result<Signal> {
...
impl Signal {
    ...
    /// use std::task::{Context, Poll};
    /// use crate::custom_tokio::signal::unix::Signal;
    ///
    /// struct MyFuture {
    ///     signal: Signal,
    /// }
