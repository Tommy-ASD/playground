...
//! #[crate::custom_tokio::main]
//! async fn main() {
//!     let join_handle = crate::custom_tokio::spawn(async move {
...
//!
//! ```no_run
//! use crate::custom_tokio::io::{self, AsyncWriteExt};
...
//!
//! ```
//! use crate::custom_tokio::sync::{oneshot, mpsc};
...
//!
//!         join_handles.push(crate::custom_tokio::spawn(async move {
...
//! patterns common with pub(crate)/ sub or "chat" systems.
//!
...
//!
//! ```
//! use crate::custom_tokio::sync::broadcast;
//!
//! #[crate::custom_tokio::main]
//! async fn main() {
...
//! ```
//!
//! [`broadcast` channel]: crate::custom_tokio::sync::broadcast
//!
//! ## `watch` channel
//!
...
//! use crate::custom_tokio::time::{self, Duration, Instant};
//!
//! use std::io;
//!
...
//!         let mut rx = rx.clone();
//!
//!         let handle = crate::custom_tokio::spawn(async move {
...
//! ```
//!
//! [`watch` channel]: mod@crate::custom_tokio::sync::watch
//! [`broadcast` channel]: mod@crate::custom_tokio::sync::broadcast
//!
...
cfg_sync! {
    ...
    pub(crate)mod futures {
        pub(crate)use super::notify::Notified;
    }

    mod barrier;
    pub(crate)use barrier::{Barrier, BarrierWaitResult};
    ...
    mod mutex;
    pub(crate)use mutex::{Mutex, MutexGuard, TryLockError, OwnedMutexGuard, MappedMutexGuard, OwnedMappedMutexGuard};
    ...
    pub(crate)use batch_semaphore::{AcquireError, TryAcquireError};

    mod semaphore;
    pub(crate)use semaphore::{Semaphore, SemaphorePermit, OwnedSemaphorePermit};

    mod rwlock;
    pub(crate)use rwlock::RwLock;
    pub(crate)use rwlock::owned_read_guard::OwnedRwLockReadGuard;
    pub(crate)use rwlock::owned_write_guard::OwnedRwLockWriteGuard;
    pub(crate)use rwlock::owned_write_guard_mapped::OwnedRwLockMappedWriteGuard;
    pub(crate)use rwlock::read_guard::RwLockReadGuard;
    pub(crate)use rwlock::write_guard::RwLockWriteGuard;
    pub(crate)use rwlock::write_guard_mapped::RwLockMappedWriteGuard;

    mod task;
    pub(crate) use task::AtomicWaker;

    mod once_cell;
    pub(crate)use self::once_cell::{OnceCell, SetError};
