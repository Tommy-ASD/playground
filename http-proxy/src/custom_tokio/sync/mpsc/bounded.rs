use crate::custom_tokio::loom::sync::Arc;
use crate::custom_tokio::sync::batch_semaphore::{self as semaphore, TryAcquireError};
use crate::custom_tokio::sync::mpsc::chan;
use crate::custom_tokio::sync::mpsc::error::{SendError, TryRecvError, TrySendError};

cfg_time! {
    use crate::custom_tokio::sync::mpsc::error::SendTimeoutError;
    use crate::custom_tokio::time::Duration;
}
...
///
/// ```
/// use crate::custom_tokio::sync::mpsc::channel;
///
...
pub(crate) struct OwnedPermit<T> {
...
impl<T> Receiver<T> {
    ...
    /// [`crate::custom_tokio::select!`](crate::custom_tokio::select) statement and some other branch
    /// completes first, it is guaranteed that no messages were received on this
    /// channel.
    ///
    /// [`close`]: Self::close
    /// [`Permits`]: struct@crate::custom_tokio::sync::mpsc::Permit
    ///
    /// # Examples
    ///
    /// ```
    ...
    ///
    /// [`Empty`]: crate::custom_tokio::sync::mpsc::error::TryRecvError::Empty
    /// [`Disconnected`]: crate::custom_tokio::sync::mpsc::error::TryRecvError::Disconnected
    /// [`poll_recv`]: Self::poll_recv
    /// [senders]: crate::custom_tokio::sync::mpsc::Sender
    /// [permits]: crate::custom_tokio::sync::mpsc::Permit
    ///
    /// # Examples
    ///
    /// ```
    /// use crate::custom_tokio::sync::mpsc;
    /// use crate::custom_tokio::sync::mpsc::error::TryRecvError;
    ///
    /// #[crate::custom_tokio::main]
    /// async fn main() {
    ///     let (tx, mut rx) = mpsc::channel(100);
    ...
    /// [`close`]: Self::close
    /// [`Permits`]: struct@crate::custom_tokio::sync::mpsc::Permit
    /// [`blocking_send`]: fn@crate::custom_tokio::sync::mpsc::Sender::blocking_send
    ///
    /// # Panics
    ///
    ...
    pub(crate) fn blocking_recv(&mut self) -> Option<T> {
        crate::custom_tokio::future::block_on(self.recv())
    }
...
impl<T> Sender<T> {
    ...
    /// # Cancel safety
    ///
    /// If `send` is used as the event in a [`crate::custom_tokio::select!`](crate::custom_tokio::select)
    ...
    /// runtime [with time enabled](crate::custom_tokio::runtime::Builder::enable_time).
    ///
    /// # Examples
    ///
    ...
    /// ```rust
    /// use crate::custom_tokio::sync::mpsc;
    /// use crate::custom_tokio::time::{sleep, Duration};
    ///
    /// #[crate::custom_tokio::main]
    /// async fn main() {
    ///     let (tx, mut rx) = mpsc::channel(1);
    ...
    pub(crate) async fn send_timeout(
        ...
        let permit = match crate::custom_tokio::time::timeout(timeout, self.reserve()).await {
            Err(_) => {
    ...
    ///
    /// [`blocking_recv`]: fn@crate::custom_tokio::sync::mpsc::Receiver::blocking_recv
    ///
    /// # Panics
    ///
    /// This function panics if called within an asynchronous execution
    ...
    pub(crate) fn blocking_send(&self, value: T) -> Result<(), SendError<T>> {
        crate::custom_tokio::future::block_on(self.send(value))
    }
    ...
    /// called.
    ///
    /// [`Receiver`]: crate::custom_tokio::sync::mpsc::Receiver
    /// [`Receiver::close`]: crate::custom_tokio::sync::mpsc::Receiver::close
    ///
    /// ```
    /// let (tx, rx) = crate::custom_tokio::sync::mpsc::channel::<()>(42);
    /// assert!(!tx.is_closed());
    ///
    /// let tx2 = tx.clone();
    ...
    /// }
    /// ```
    pub(crate) async fn reserve(&self) -> Result<Permit<'_, T>, SendError<()>> {
    ...
    pub(crate) async fn reserve_owned(self) -> Result<OwnedPermit<T>, SendError<()>> {
        self.reserve_inner().await?;
        Ok(OwnedPermit {
            chan: Some(self.chan),
        })
    ...
    async fn reserve_inner(&self) -> Result<(), SendError<()>> {
        crate::custom_tokio::trace::async_trace_leaf().await;
    ...
    /// }
    /// ```
    pub(crate) fn try_reserve_owned(self) -> Result<OwnedPermit<T>, TrySendError<Self>> {
        match self.chan.semaphore().semaphore.try_acquire(1) {
            Ok(_) => {}
    ...
    ///
    /// ```
    /// let (tx, rx) = crate::custom_tokio::sync::mpsc::channel::<()>(1);
    /// let  tx2 = tx.clone();
    /// assert!(tx.same_channel(&tx2));
    ///
    /// let (tx3, rx3) = crate::custom_tokio::sync::mpsc::channel::<()>(1);
...
impl<T> OwnedPermit<T> {
    ...
    /// }
    /// ```
    pub(crate) fn send(mut self, value: T) -> Sender<T> {
    ...
    ///
    /// [`Sender`]: Sender
    pub(crate) fn release(mut self) -> Sender<T> {
        use chan::Semaphore;
