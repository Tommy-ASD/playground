...
use crate::custom_tokio::loom::future::AtomicWaker;
use crate::custom_tokio::loom::sync::atomic::AtomicUsize;
use crate::custom_tokio::loom::sync::Arc;
use crate::custom_tokio::runtime::park::CachedParkThread;
use crate::custom_tokio::sync::mpsc::error::TryRecvError;
use crate::custom_tokio::sync::mpsc::{bounded, list, unbounded};
use crate::custom_tokio::sync::notify::Notify;
use crate::custom_tokio::util::cacheline::CachePadded;
...
impl<T, S> Tx<T, S> {
    ...
    pub(crate) fn downgrade(&self) -> Arc<Chan<T, S>> {
        self.inner.clone()
    }

    // Returns the upgraded channel or None if the upgrade failed.
    pub(crate) fn upgrade(chan: Arc<Chan<T, S>>) -> Option<Self> {
