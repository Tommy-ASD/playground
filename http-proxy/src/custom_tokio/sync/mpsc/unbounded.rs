use crate::custom_tokio::loom::sync::{atomic::AtomicUsize, Arc};
use crate::custom_tokio::sync::mpsc::chan;
use crate::custom_tokio::sync::mpsc::error::{SendError, TryRecvError};
...
///
/// ```
/// use crate::custom_tokio::sync::mpsc::unbounded_channel;
///
...
/// }
/// ```
pub(crate) struct WeakUnboundedSender<T> {
...
impl<T> UnboundedSender<T> {
    ...
    ///
    /// [`UnboundedReceiver`]: crate::custom_tokio::sync::mpsc::UnboundedReceiver
    /// [`UnboundedReceiver::close`]: crate::custom_tokio::sync::mpsc::UnboundedReceiver::close
    ///
    /// ```
    /// let (tx, rx) = crate::custom_tokio::sync::mpsc::unbounded_channel::<()>();
    /// assert!(!tx.is_closed());
    ///
    /// let tx2 = tx.clone();
    ...
    ///
    /// let (tx3, rx3) = crate::custom_tokio::sync::mpsc::unbounded_channel::<()>();
    ...
    /// the channel is closed.
    pub(crate) fn downgrade(&self) -> WeakUnboundedSender<T> {
        WeakUnboundedSender {
...
impl<T> WeakUnboundedSender<T> {
    ...
    pub(crate) fn upgrade(&self) -> Option<UnboundedSender<T>> {
