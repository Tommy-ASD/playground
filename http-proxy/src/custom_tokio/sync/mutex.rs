...
use crate::custom_tokio::sync::batch_semaphore as semaphore;
#[cfg(all(tokio_unstable, feature = "tracing"))]
use crate::custom_tokio::util::trace;
...
///
/// ```rust,no_run
/// use crate::custom_tokio::sync::Mutex;
/// use std::sync::Arc;
///
...
/// [`Arc`]: std::sync::Arc
#[clippy::has_significant_drop]
pub(crate) struct OwnedMutexGuard<T: ?Sized> {
    // When changing the fields in this struct, make sure to update the
    // `skip_drop` method.
    #[cfg(all(tokio_unstable, feature = "tracing"))]
    resource_span: tracing::Span,
...
#[clippy::has_significant_drop]
#[must_use = "if unused the Mutex will immediately unlock"]
pub(crate) struct MappedMutexGuard<'a, T: ?Sized> {
    // When changing the fields in this struct, make sure to update the
    // `skip_drop` method.
    #[cfg(all(tokio_unstable, feature = "tracing"))]
    resource_span: tracing::Span,
...
#[clippy::has_significant_drop]
#[must_use = "if unused the Mutex will immediately unlock"]
pub(crate) struct OwnedMappedMutexGuard<T: ?Sized, U: ?Sized = T> {
    // When changing the fields in this struct, make sure to update the
    // `skip_drop` method.
    #[cfg(all(tokio_unstable, feature = "tracing"))]
    resource_span: tracing::Span,
...
#[derive(Debug)]
pub(crate) struct TryLockError(pub(crate) ());
...
impl<T: ?Sized> Mutex<T> {
    ...
    ///     [`spawn_blocking()`][crate::custom_tokio::runtime::Handle::spawn_blocking]
    ///     (or [`block_in_place()`][crate::custom_tokio::task::block_in_place]).
    ///
    /// # Examples
    ///
    /// ```
    ...
    ///     let blocking_task = crate::custom_tokio::task::spawn_blocking(move || {
    ...
    pub(crate) fn blocking_lock(&self) -> MutexGuard<'_, T> {
        crate::custom_tokio::future::block_on(self.lock())
    }
    ...
    #[track_caller]
    #[cfg(feature = "sync")]
    pub(crate) fn blocking_lock_owned(self: Arc<Self>) -> OwnedMutexGuard<T> {
        crate::custom_tokio::future::block_on(self.lock_owned())
    }
    ...
    ///
    /// [`Arc`]: std::sync::Arc
    pub(crate) async fn lock_owned(self: Arc<Self>) -> OwnedMutexGuard<T> {
        #[cfg(all(tokio_unstable, feature = "tracing"))]
    ...
    /// ```
    /// use crate::custom_tokio::sync::Mutex;
    /// # async fn dox() -> Result<(), crate::custom_tokio::sync::TryLockError> {
    ///
    /// let mutex = Mutex::new(1);
    ///
    ...
    /// # }
    /// ```
    pub(crate) fn try_lock(&self) -> Result<MutexGuard<'_, T>, TryLockError> {
    ...
    /// # Ok(())
    /// # }
    pub(crate) fn try_lock_owned(self: Arc<Self>) -> Result<OwnedMutexGuard<T>, TryLockError> {
        match self.s.try_acquire(1) {
            Ok(_) => {
...
impl<'a, T: ?Sized> MutexGuard<'a, T> {
    ...
    ///
    /// ```
    /// use crate::custom_tokio::sync::{Mutex, MutexGuard};
    ///
    ...
    #[inline]
    pub(crate) fn map<U, F>(mut this: Self, f: F) -> MappedMutexGuard<'a, U>
    where
    ...
    /// [`MappedMutexGuard`]: struct@MappedMutexGuard
    #[inline]
    pub(crate) fn try_map<U, F>(mut this: Self, f: F) -> Result<MappedMutexGuard<'a, U>, Self>
    where
...
impl<T: ?Sized> OwnedMutexGuard<T> {
    ...
    ///
    /// ```
    /// use crate::custom_tokio::sync::{Mutex, OwnedMutexGuard};
    /// use std::sync::Arc;
    ///
    /// #[derive(Debug, Clone, Copy, PartialEq, Eq)]
    /// struct Foo(u32);
    ...
    #[inline]
    pub(crate) fn map<U, F>(mut this: Self, f: F) -> OwnedMappedMutexGuard<T, U>
    where
        F: FnOnce(&mut T) -> &mut U,
    {
        let data = f(&mut *this) as *mut U;
    ...
    /// [`OwnedMappedMutexGuard`]: struct@OwnedMappedMutexGuard
    #[inline]
    pub(crate) fn try_map<U, F>(mut this: Self, f: F) -> Result<OwnedMappedMutexGuard<T, U>, Self>
    where
        F: FnOnce(&mut T) -> Option<&mut U>,
    {
        let data = match f(&mut *this) {
    ...
    /// ```
    #[inline]
    pub(crate) fn mutex(this: &Self) -> &Arc<Mutex<T>> {
        &this.lock
    }
}
...
impl<T: ?Sized, U: ?Sized> OwnedMappedMutexGuard<T, U> {
    ...
    /// [`OwnedMappedMutexGuard`]: struct@OwnedMappedMutexGuard
    #[inline]
    pub(crate) fn map<S, F>(mut this: Self, f: F) -> OwnedMappedMutexGuard<T, S>
    where
    ...
    /// [`OwnedMappedMutexGuard`]: struct@OwnedMappedMutexGuard
    #[inline]
    pub(crate) fn try_map<S, F>(mut this: Self, f: F) -> Result<OwnedMappedMutexGuard<T, S>, Self>
    where
