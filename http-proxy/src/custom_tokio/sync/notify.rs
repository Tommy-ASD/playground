...
use crate::custom_tokio::loom::sync::Mutex;
...
/// [`Semaphore`]: crate::custom_tokio::sync::Semaphore
#[derive(Debug)]
...
impl Notify {
    ...
    pub(crate) const fn const_new() -> Notify {
        Notify {
            state: AtomicUsize::new(0),
...
impl Notified<'_> {
    ...
    pub(crate) fn enable(self: Pin<&mut Self>) -> bool {
            ...
            match *state {
                ...
                Waiting => {
                    ...
                    if let Some(waker) = waker {
                        ...
                        ready!(crate::custom_tokio::trace::trace_leaf(&mut ctx));
                    }
