...
//! ```
//!
//! To use a oneshot channel in a `crate::custom_tokio::select!` loop, add `&mut` in front of
//! the channel.
//!
//! ```
//! use crate::custom_tokio::sync::oneshot;
//! use crate::custom_tokio::time::{interval, sleep, Duration};
//!
//! #[crate::custom_tokio::main]
//! # async fn _doc() {}
//! # #[crate::custom_tokio::main(flavor = "current_thread", start_paused = true)]
//! async fn main() {
...
/// ```
///
/// To use a `Receiver` in a `crate::custom_tokio::select!` loop, add `&mut` in front of the
/// channel.
///
/// ```
/// use crate::custom_tokio::sync::oneshot;
...
impl<T> Sender<T> {
    ...
    /// }
    /// ```
    pub(crate) fn send(mut self, t: T) -> Result<(), T> {
    ...
    ///
    /// [`Receiver`]: struct@crate::custom_tokio::sync::oneshot::Receiver
    /// [`close`]: fn@crate::custom_tokio::sync::oneshot::Receiver::close
    ///
    /// # Return value
    ///
...
impl<T> Receiver<T> {
    ...
    /// ```
    /// use crate::custom_tokio::sync::oneshot;
    /// use crate::custom_tokio::sync::oneshot::error::TryRecvError;
    ///
    /// #[crate::custom_tokio::main]
    /// async fn main() {
    ...
    pub(crate) fn blocking_recv(self) -> Result<T, RecvError> {
        crate::custom_tokio::future::block_on(self)
    }
}
