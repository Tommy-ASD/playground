use crate::custom_tokio::sync::batch_semaphore::{Semaphore, TryAcquireError};
use crate::custom_tokio::sync::mutex::TryLockError;
...
///
/// ```
/// use crate::custom_tokio::sync::RwLock;
///
...
impl<T: ?Sized> RwLock<T> {
    ...
    #[track_caller]
    pub(crate) fn with_max_readers(value: T, max_reads: u32) -> RwLock<T>
    where
        T: Sized,
    {
        assert!(
    ...
    pub(crate) const fn const_new(value: T) -> RwLock<T>
    where
        T: Sized,
    {
        RwLock {
    ...
    /// ```
    #[cfg(not(all(loom, test)))]
    pub(crate) const fn const_with_max_readers(value: T, max_reads: u32) -> RwLock<T>
    where
        T: Sized,
    {
    ...
    #[track_caller]
    #[cfg(feature = "sync")]
    pub(crate) fn blocking_read(&self) -> RwLockReadGuard<'_, T> {
        crate::custom_tokio::future::block_on(self.read())
    }
    ...
    ///}
    /// ```
    pub(crate) async fn read_owned(self: Arc<Self>) -> OwnedRwLockReadGuard<T> {
        #[cfg(all(tokio_unstable, feature = "tracing"))]
    ...
    /// }
    /// ```
    pub(crate) fn try_read_owned(self: Arc<Self>) -> Result<OwnedRwLockReadGuard<T>, TryLockError> {
        match self.s.try_acquire(1) {
            Ok(permit) => permit,
            Err(TryAcquireError::NoPermits) => return Err(TryLockError(())),
            Err(TryAcquireError::Closed) => unreachable!(),
    ...
    #[track_caller]
    #[cfg(feature = "sync")]
    pub(crate) fn blocking_write(&self) -> RwLockWriteGuard<'_, T> {
        crate::custom_tokio::future::block_on(self.write())
    }
    ...
    ///}
    /// ```
    pub(crate) async fn write_owned(self: Arc<Self>) -> OwnedRwLockWriteGuard<T> {
        #[cfg(all(tokio_unstable, feature = "tracing"))]
        let resource_span = self.resource_span.clone();
    ...
    /// }
    /// ```
    pub(crate) fn try_write(&self) -> Result<RwLockWriteGuard<'_, T>, TryLockError> {
