use crate::custom_tokio::sync::rwlock::owned_read_guard::OwnedRwLockReadGuard;
use crate::custom_tokio::sync::rwlock::owned_write_guard_mapped::OwnedRwLockMappedWriteGuard;
...
impl<T: ?Sized> OwnedRwLockWriteGuard<T> {
    ...
    /// ```
    /// use std::sync::Arc;
    /// use crate::custom_tokio::sync::{RwLock, OwnedRwLockWriteGuard};
    ///
    ...
    /// ```
    #[inline]
    pub(crate) fn map<F, U: ?Sized>(mut this: Self, f: F) -> OwnedRwLockMappedWriteGuard<T, U>
    where
    ...
    /// ```
    #[inline]
    pub(crate) fn downgrade_map<F, U: ?Sized>(this: Self, f: F) -> OwnedRwLockReadGuard<T, U>
    where
        F: FnOnce(&T) -> &U,
    {
    ...
    ///
    /// [`RwLockMappedWriteGuard`]: struct@crate::custom_tokio::sync::RwLockMappedWriteGuard
    ///
    /// # Examples
    ///
    /// ```
    ...
    /// ```
    #[inline]
    pub(crate) fn try_map<F, U: ?Sized>(
        mut this: Self,
        f: F,
    ...
    /// ```
    #[inline]
    pub(crate) fn try_downgrade_map<F, U: ?Sized>(
        this: Self,
        f: F,
    ...
    #[inline]
    pub(crate) fn into_mapped(this: Self) -> OwnedRwLockMappedWriteGuard<T> {
        Self::map(this, |me| me)
    }
    ...
    /// # }
    /// ```
    pub(crate) fn downgrade(self) -> OwnedRwLockReadGuard<T> {
