...
/// [`RwLock`].
///
/// [`read`]: method@crate::custom_tokio::sync::RwLock::read
...
impl<'a, T: ?Sized> RwLockReadGuard<'a, T> {
    ...
    ///
    /// ```
    /// use crate::custom_tokio::sync::{RwLock, RwLockReadGuard};
    ///
    ...
    /// ```
    #[inline]
    pub(crate) fn map<F, U: ?Sized>(this: Self, f: F) -> RwLockReadGuard<'a, U>
    where
        F: FnOnce(&T) -> &U,
    {
    ...
    /// ```
    #[inline]
    pub(crate) fn try_map<F, U: ?Sized>(this: Self, f: F) -> Result<RwLockReadGuard<'a, U>, Self>
    where
        F: FnOnce(&T) -> Option<&U>,
    {
