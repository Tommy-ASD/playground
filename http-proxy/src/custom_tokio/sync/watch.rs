...
//!
//! ```
//! use crate::custom_tokio::sync::watch;
//!
...
//!
//! [`Sender`]: crate::custom_tokio::sync::watch::Sender
//! [`Receiver`]: crate::custom_tokio::sync::watch::Receiver
//! [`Receiver::changed()`]: crate::custom_tokio::sync::watch::Receiver::changed
//! [`Receiver::borrow()`]: crate::custom_tokio::sync::watch::Receiver::borrow
//! [`channel`]: crate::custom_tokio::sync::watch::channel
//! [`Sender::is_closed`]: crate::custom_tokio::sync::watch::Sender::is_closed
//! [`Sender::closed`]: crate::custom_tokio::sync::watch::Sender::closed

use crate::custom_tokio::sync::notify::Notify;

use crate::custom_tokio::loom::sync::atomic::AtomicUsize;
use crate::custom_tokio::loom::sync::atomic::Ordering::Relaxed;
use crate::custom_tokio::loom::sync::{Arc, RwLock, RwLockReadGuard};
use std::fmt;
use std::mem;
use std::ops;
use std::panic;
...
pub(crate) mod error {
    ...
    #[derive(Debug, Clone)]
    pub(crate) struct RecvError(pub(crate) ());
...
mod big_notify {
    use super::*;
    use crate::custom_tokio::sync::notify::Notified;
    ...
    impl BigNotify {
        ...
        pub(crate) fn notified(&self) -> Notified<'_> {
            let i = crate::custom_tokio::runtime::context::thread_rng_n(8) as usize;
            self.inner[i].notified()
        }
    }
}
...
mod state {
    use crate::custom_tokio::loom::sync::atomic::AtomicUsize;
    use crate::custom_tokio::loom::sync::atomic::Ordering::SeqCst;
    ...
    #[derive(Debug)]
    pub(crate) struct AtomicState(AtomicUsize);
...
pub(crate) fn channel<T>(init: T) -> (Sender<T>, Receiver<T>) {
...
impl<T> Receiver<T> {
    ...
    ///
    /// [`changed`]: Receiver::changed
    pub(crate) fn borrow_and_update(&mut self) -> Ref<'_, T> {
        let inner = self.shared.value.read().unwrap();
    ...
    /// }
    /// ```
    pub(crate) fn has_changed(&self) -> Result<bool, error::RecvError> {
    ...
    /// }
    /// ```
    pub(crate) async fn changed(&mut self) -> Result<(), error::RecvError> {
    ...
    ///
    /// ```
    /// let (tx, rx) = crate::custom_tokio::sync::watch::channel(true);
    /// let rx2 = rx.clone();
    /// assert!(rx.same_channel(&rx2));
    ///
    /// let (tx3, rx3) = crate::custom_tokio::sync::watch::channel(true);
...
impl<T> Sender<T> {
    ...
    pub(crate) fn send(&self, value: T) -> Result<(), error::SendError<T>> {
    ...
    /// ```
    pub(crate) fn send_modify<F>(&self, modify: F)
    where
        F: FnOnce(&mut T),
    {
    ...
    /// assert_eq!(state_rx.borrow_and_update().counter, 2);
    /// ```
    pub(crate) fn send_if_modified<F>(&self, modify: F) -> bool
    where
        F: FnOnce(&mut T) -> bool,
    {
        {
    ...
    /// ```
    pub(crate) fn send_replace(&self, mut value: T) -> T {
    ...
    ///
    /// ```
    /// let (tx, rx) = crate::custom_tokio::sync::watch::channel(());
    /// assert!(!tx.is_closed());
    ///
    /// drop(rx);
    /// assert!(tx.is_closed());
    ...
    ///         # if false {
    ///         crate::custom_tokio::time::sleep(Duration::from_millis(10)).await;
    ///         # }
    ///         tx.send(100).unwrap();
    ///     });
    ///
...
mod tests {
    ...
    fn watch_spurious_wakeup() {
        loom::model(|| {
            let (send, mut recv) = crate::custom_tokio::sync::watch::channel(0i32);
