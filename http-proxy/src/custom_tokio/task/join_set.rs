...
#[cfg(tokio_unstable)]
use crate::custom_tokio::task::Id;
use crate::custom_tokio::task::{AbortHandle, JoinError, JoinHandle, LocalSet};
use crate::custom_tokio::util::IdleNotifiedSet;
...
///
/// ```
/// use crate::custom_tokio::task::JoinSet;
///
...
///
/// [`task::Builder`]: crate::custom_tokio::task::Builder
...
impl<T: 'static> JoinSet<T> {
    ...
    #[cfg(all(tokio_unstable, feature = "tracing"))]
    #[cfg_attr(docsrs, doc(cfg(all(tokio_unstable, feature = "tracing"))))]
    pub(crate) fn build_task(&mut self) -> Builder<'_, T> {
        Builder {
    ...
    ///
    /// [`AbortHandle`]: crate::custom_tokio::task::AbortHandle
    #[track_caller]
    pub(crate) fn spawn<F>(&mut self, task: F) -> AbortHandle
    where
        F: Future<Output = T>,
        F: Send + 'static,
        T: Send,
    {
        self.insert(crate::custom_tokio::spawn(task))
    }
    ...
    /// [`AbortHandle`]: crate::custom_tokio::task::AbortHandle
    #[track_caller]
    pub(crate) fn spawn_on<F>(&mut self, task: F, handle: &Handle) -> AbortHandle
    where
        F: Future<Output = T>,
        F: Send + 'static,
        T: Send,
    ...
    /// [`AbortHandle`]: crate::custom_tokio::task::AbortHandle
    #[track_caller]
    pub(crate) fn spawn_local<F>(&mut self, task: F) -> AbortHandle
    where
        F: Future<Output = T>,
        F: 'static,
    {
        self.insert(crate::custom_tokio::task::spawn_local(task))
    }
    ...
    #[track_caller]
    pub(crate) fn spawn_local_on<F>(&mut self, task: F, local_set: &LocalSet) -> AbortHandle
    where
        F: Future<Output = T>,
        F: 'static,
    {
    ...
    /// [`AbortHandle`]: crate::custom_tokio::task::AbortHandle
    #[track_caller]
    pub(crate) fn spawn_blocking<F>(&mut self, f: F) -> AbortHandle
    where
        F: FnOnce() -> T,
        F: Send + 'static,
        T: Send,
    {
        self.insert(crate::custom_tokio::runtime::spawn_blocking(f))
    }
    ...
    /// [`AbortHandle`]: crate::custom_tokio::task::AbortHandle
    #[track_caller]
    pub(crate) fn spawn_blocking_on<F>(&mut self, f: F, handle: &Handle) -> AbortHandle
    where
        F: FnOnce() -> T,
        F: Send + 'static,
        T: Send,
    ...
    /// # Cancel Safety
    ///
    /// This method is cancel safe. If `join_next` is used as the event in a `crate::custom_tokio::select!`
    /// statement and some other branch completes first, it is guaranteed that no tasks were
    /// removed from this `JoinSet`.
    pub(crate) async fn join_next(&mut self) -> Option<Result<T, JoinError>> {
        crate::custom_tokio::future::poll_fn(|cx| self.poll_join_next(cx)).await
    }
    ...
    /// # Cancel Safety
    ///
    /// This method is cancel safe. If `join_next_with_id` is used as the event in a `crate::custom_tokio::select!`
    /// statement and some other branch completes first, it is guaranteed that no tasks were
    /// removed from this `JoinSet`.
    ///
    /// [task ID]: crate::custom_tokio::task::Id
    /// [`JoinError::id`]: fn@crate::custom_tokio::task::JoinError::id
    #[cfg(tokio_unstable)]
    #[cfg_attr(docsrs, doc(cfg(tokio_unstable)))]
    pub(crate) async fn join_next_with_id(&mut self) -> Option<Result<(Id, T), JoinError>> {
        crate::custom_tokio::future::poll_fn(|cx| self.poll_join_next_with_id(cx)).await
    }
    ...
    ///
    /// [coop budget]: crate::custom_tokio::task#cooperative-scheduling
    ...
    #[cfg(tokio_unstable)]
    #[cfg_attr(docsrs, doc(cfg(tokio_unstable)))]
    pub(crate) fn poll_join_next_with_id(
        &mut self,
        cx: &mut Context<'_>,
...
impl<'a, T: 'static> Builder<'a, T> {
    ...
    /// [`AbortHandle`]: crate::custom_tokio::task::AbortHandle
    #[track_caller]
    pub(crate) fn spawn<F>(self, future: F) -> std::io::Result<AbortHandle>
    where
        F: Future<Output = T>,
        F: Send + 'static,
        T: Send,
    ...
    #[track_caller]
    pub(crate) fn spawn_on<F>(self, future: F, handle: &Handle) -> std::io::Result<AbortHandle>
    where
        F: Future<Output = T>,
        F: Send + 'static,
        T: Send,
    ...
    /// [`AbortHandle`]: crate::custom_tokio::task::AbortHandle
    #[track_caller]
    pub(crate) fn spawn_local<F>(self, future: F) -> std::io::Result<AbortHandle>
    where
        F: Future<Output = T>,
        F: 'static,
    {
