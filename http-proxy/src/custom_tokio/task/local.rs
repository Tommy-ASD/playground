...
use crate::custom_tokio::loom::cell::UnsafeCell;
use crate::custom_tokio::loom::sync::{Arc, Mutex};
#[cfg(tokio_unstable)]
use crate::custom_tokio::runtime;
use crate::custom_tokio::runtime::task::{self, JoinHandle, LocalOwnedTasks, Task};
use crate::custom_tokio::runtime::{context, ThreadId};
use crate::custom_tokio::sync::AtomicWaker;
use crate::custom_tokio::util::RcCell;
...
cfg_rt! {
    ...
    ///     // Since `crate::custom_tokio::spawn` requires the spawned future to implement `Send`, this
    ///     // will not compile.
    ...
    /// }
    /// ```
    /// **Note:** The `run_until` method can only be used in `#[crate::custom_tokio::main]`,
    /// `#[tokio::test]` or directly inside a call to [`Runtime::block_on`]. It
    /// cannot be used inside a task spawned with `crate::custom_tokio::spawn`.
    ///
    /// ## Awaiting a `LocalSet`
    ///
    ...
    ///
    /// ```rust
    /// use crate::custom_tokio::{task, time};
    /// use std::rc::Rc;
    ///
    /// #[crate::custom_tokio::main]
    /// async fn main() {
    ...
    /// `#[crate::custom_tokio::main]`, `#[tokio::test]` or directly inside a call to
    /// [`Runtime::block_on`]. It cannot be used inside a task spawned with
    /// `crate::custom_tokio::spawn`.
    ///
    /// ## Use inside `crate::custom_tokio::spawn`
    ///
    /// The two methods mentioned above cannot be used inside `crate::custom_tokio::spawn`, so
    /// to spawn `!Send` futures from inside `crate::custom_tokio::spawn`, we need to do
    ...
    /// use crate::custom_tokio::sync::{mpsc, oneshot};
    /// use crate::custom_tokio::task::LocalSet;
    ///
    ...
    ///                     crate::custom_tokio::task::spawn_local(run_task(new_task));
    ///                 }
    ...
    /// [`Runtime::block_on`]: method@crate::custom_tokio::runtime::Runtime::block_on
    /// [`task::spawn_local`]: fn@spawn_local
    /// [`mpsc`]: mod@crate::custom_tokio::sync::mpsc
...
struct Shared {
    ...
    #[cfg(tokio_unstable)]
    pub(crate) unhandled_panic: crate::custom_tokio::runtime::UnhandledPanic,
}
...
cfg_rt! {
    ...
    ///
    /// Note that if [`crate::custom_tokio::spawn`] is used from within a `LocalSet`, the
    ...
    /// ```
    ///
    /// [`LocalSet`]: struct@crate::custom_tokio::task::LocalSet
    /// [`crate::custom_tokio::spawn`]: fn@crate::custom_tokio::task::spawn
    #[track_caller]
    ...
    #[track_caller]
    pub(crate) fn spawn_local_inner<F>(future: F, name: Option<&str>) -> JoinHandle<F::Output>
...
pub(crate) struct LocalEnterGuard(Option<Rc<Context>>);
...
impl LocalSet {
        ...
        LocalSet {
            ...
            context: Rc::new(Context {
                shared: Arc::new(Shared {
                    ...
                    #[cfg(tokio_unstable)]
                    unhandled_panic: crate::custom_tokio::runtime::UnhandledPanic::Ignore,
                }),
    ...
    ///
    /// [`spawn_local`]: fn@crate::custom_tokio::task::spawn_local
    pub(crate) fn enter(&self) -> LocalEnterGuard {
        CURRENT.with(|LocalData { ctx, .. }| {
    ...
    /// [`spawn_local`]: fn@spawn_local
    /// [`Runtime::block_on`]: method@crate::custom_tokio::runtime::Runtime::block_on
    /// [in-place blocking]: fn@crate::custom_tokio::task::block_in_place
    ...
    pub(crate) fn block_on<F>(
        &self,
        rt: &crate::custom_tokio::runtime::Runtime,
        future: F,
    ) -> F::Output
    where
        F: Future,
    ...
    pub(crate) async fn run_until<F>(&self, future: F) -> F::Output
    where
        F: Future,
    {
    ...
    pub(in crate::custom_tokio::task) fn spawn_named<F>(
        &self,
        future: F,
        name: Option<&str>,
    ) -> JoinHandle<F::Output>
    ...
    fn tick(&self) -> bool {
        for _ in 0..MAX_TASKS_PER_TICK {
            ...
            match self.next_task() {
                ...
                Some(task) => crate::custom_tokio::runtime::coop::budget(|| task.run()),
...
cfg_unstable! {
    impl LocalSet {
        ...
        ///
        /// ```should_panic
        /// use crate::custom_tokio::runtime::UnhandledPanic;
        ///
        /// # #[crate::custom_tokio::main]
        /// # async fn main() {
        /// crate::custom_tokio::task::LocalSet::new()
        ///     .unhandled_panic(UnhandledPanic::ShutdownRuntime)
        ///     .run_until(async {
        ///         crate::custom_tokio::task::spawn_local(async { panic!("boom"); });
        ///         crate::custom_tokio::task::spawn_local(async {
        ...
        pub(crate)fn unhandled_panic(&mut self, behavior: crate::custom_tokio::runtime::UnhandledPanic) -> &mut Self {
...
impl Context {
        ...
        F::Output: 'static,
    {
        ...
        let future = crate::custom_tokio::util::trace::task(future, "local", name, id.as_u64());
...
impl<T: Future> Future for RunUntil<'_, T> {
    ...
    fn poll(self: Pin<&mut Self>, cx: &mut std::task::Context<'_>) -> Poll<Self::Output> {
        ...
        me.local_set.with(|| {
            ...
            let _no_blocking = crate::custom_tokio::runtime::context::disallow_block_in_place();
            let f = me.future;
...
mod tests {
    ...
    fn local_current_thread_scheduler() {
        ...
        crate::custom_tokio::runtime::Builder::new_current_thread()
            .build()
            .expect("rt")
            .block_on(f)
    }
    ...
    fn wakes_to_local_queue() {
        use super::*;
        use crate::custom_tokio::sync::Notify;
        let rt = crate::custom_tokio::runtime::Builder::new_current_thread()
            .build()
            .expect("rt");
        rt.block_on(async {
            let local = LocalSet::new();
            ...
            crate::custom_tokio::future::poll_fn(|cx| {
