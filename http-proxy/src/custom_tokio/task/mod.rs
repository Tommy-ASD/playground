...
//! use crate::custom_tokio::task;
//!
//! # #[crate::custom_tokio::main] async fn main() -> Result<(), Box<dyn std::error::Error>> {
...
//! use crate::custom_tokio::task;
//!
//! # #[crate::custom_tokio::main] async fn main() {
//! let join = task::spawn(async {
...
//! # async fn main() {
//! use crate::custom_tokio::{task, sync::mpsc};
//!
//! let fut = async {
...
//! ```
//!
//! [`task::spawn_blocking`]: crate::custom_tokio::task::spawn_blocking
//! [`task::block_in_place`]: crate::custom_tokio::task::block_in_place
//! [rt-multi-thread]: ../runtime/index.html#threaded-scheduler
//! [`task::yield_now`]: crate::custom_tokio::task::yield_now()
//! [`thread::yield_now`]: std::thread::yield_now
//! [`task::unconstrained`]: crate::custom_tokio::task::unconstrained()
//! [`poll`]: method@std::future::Future::poll

cfg_rt! {
    pub(crate)use crate::custom_tokio::runtime::task::{JoinError, JoinHandle};
    ...
    cfg_rt_multi_thread! {
        pub(crate)use blocking::block_in_place;
    }

    mod yield_now;
    pub(crate)use yield_now::yield_now;

    cfg_unstable! {
        mod consume_budget;
        pub(crate)use consume_budget::consume_budget;
    }

    mod local;
    pub(crate)use local::{spawn_local, LocalSet, LocalEnterGuard};

    mod task_local;
    pub(crate)use task_local::LocalKey;

    mod unconstrained;
    pub(crate)use unconstrained::{unconstrained, Unconstrained};

    #[doc(inline)]
    pub(crate)use join_set::JoinSet;
    pub(crate)use crate::custom_tokio::runtime::task::AbortHandle;
    ...
    cfg_unstable! {
        pub(crate)use crate::custom_tokio::runtime::task::{Id, id, try_id};
    }
    ...
    pub(crate)mod futures {
        pub(crate)use super::task_local::TaskLocalFuture;
    }
}
