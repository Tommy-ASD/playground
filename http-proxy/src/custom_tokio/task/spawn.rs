...
cfg_rt! {
    ...
    /// [`Runtime`](crate::custom_tokio::runtime::Runtime) configuration.
    ///
    ...
    /// using the [`Runtime::enter`](crate::custom_tokio::runtime::Runtime::enter()) method.
    ///
    /// # Examples
    ///
    ...
    /// ```
    /// # #[crate::custom_tokio::main(flavor = "current_thread")] async fn main() {
    ...
    ///     // immediately.
    ///     tasks.push(crate::custom_tokio::spawn(my_background_op(op)));
    /// }
    ///
    ...
    ///
    /// [`JoinSet`]: struct@crate::custom_tokio::task::JoinSet
    ///
    /// # Panics
    ///
    ...
    #[track_caller]
    pub(crate) fn spawn_inner<T>(future: T, name: Option<&str>) -> JoinHandle<T::Output>
    where
        T: Future + Send + 'static,
        T::Output: Send + 'static,
    {
        use crate::custom_tokio::runtime::{context, task};
        ...
        let id = task::Id::next();
        let task = crate::custom_tokio::util::trace::task(future, "task", name, id.as_u64());
