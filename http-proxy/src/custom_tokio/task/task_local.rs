...
/// Declares a new task-local key of type [`crate::custom_tokio::task::LocalKey`].
///
/// # Syntax
///
...
///
/// ```
/// # use crate::custom_tokio::task_local;
/// task_local! {
///     pub(crate)static ONE: u32;
///
///     #[allow(unused)]
///     static TWO: f32;
/// }
...
/// ```
///
/// See [LocalKey documentation][`crate::custom_tokio::task::LocalKey`] for more
/// information.
///
/// [`crate::custom_tokio::task::LocalKey`]: struct@crate::custom_tokio::task::LocalKey
#[macro_export]
...
macro_rules! task_local {
    ...
    ($(#[$attr:meta])* $vis:vis static $name:ident: $t:ty; $($rest:tt)*) => {
        $crate::custom_tokio::__task_local_inner!($(#[$attr])* $vis $name, $t);
        $crate::custom_tokio::task_local!($($rest)*);
    };
...
macro_rules! __task_local_inner {
    ($(#[$attr:meta])* $vis:vis $name:ident, $t:ty) => {
        $(#[$attr])*
        $vis static $name: $crate::custom_tokio::task::LocalKey<$t> = {
            std::thread_local! {
                static __KEY: std::cell::RefCell<Option<$t>> = const { std::cell::RefCell::new(None) };
            }

            $crate::custom_tokio::task::LocalKey { inner: __KEY }
        };
    };
}
...
///
/// Unlike [`std::thread::LocalKey`], `crate::custom_tokio::task::LocalKey` will
...
pub(crate) struct LocalKey<T: 'static> {
    #[doc(hidden)]
    pub(crate) inner: thread::LocalKey<RefCell<Option<T>>>,
}
...
impl<T: 'static> LocalKey<T> {
    ...
    pub(crate) fn scope<F>(&'static self, value: T, f: F) -> TaskLocalFuture<T, F>
    where
        F: Future,
    {
        TaskLocalFuture {
    ...
    /// [`try_with`]: fn@Self::try_with
    #[track_caller]
    pub(crate) fn sync_scope<F, R>(&'static self, value: T, f: F) -> R
    where
        F: FnOnce() -> R,
    {
...
pin_project! {
    ...
    /// # }
    /// ```
    pub(crate)struct TaskLocalFuture<T, F>
    where
        T: 'static,
    {
        local: &'static LocalKey<T>,
