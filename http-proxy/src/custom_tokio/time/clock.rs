...
cfg_rt! {
    ...
    fn with_clock<R>(f: impl FnOnce(Option<&Clock>) -> Result<R, &'static str>) -> R {
        ...
        let res = match Handle::try_current() {
            ...
            Err(_) => panic!("{}", crate::custom_tokio::util::error::THREAD_LOCAL_DESTROYED_ERROR),
        };
...
/// [`advance`]: crate::custom_tokio::time::advance
#[track_caller]
pub(crate) fn pause() {
...
/// details.
///
/// [`sleep`]: fn@crate::custom_tokio::time::sleep
...
impl Clock {
    ...
    /// Temporarily stop auto-advancing the clock (see `crate::custom_tokio::time::pause`).
