...
impl Instant {
    ...
    /// Create a `crate::custom_tokio::time::Instant` from a `std::time::Instant`.
    pub(crate) fn from_std(std: std::time::Instant) -> Instant {
        Instant { std }
    }
    ...
    ///
    /// ```
    /// use crate::custom_tokio::time::{Duration, Instant, sleep};
    ///
    ...
    pub(crate) fn checked_add(&self, duration: Duration) -> Option<Instant> {
    ...
    /// represented as `Instant` (which means it's inside the bounds of the
    /// underlying data structure), `None` otherwise.
    pub(crate) fn checked_sub(&self, duration: Duration) -> Option<Instant> {
...
mod variant {
    ...
    pub(crate) fn now() -> Instant {
        crate::custom_tokio::time::clock::now()
    }
}
