...
use crate::hyper::common::{task, watch, Pin, Poll};
#[cfg(all(feature = "http2", any(feature = "client", feature = "server")))]
use crate::hyper::proto::h2::ping;

type BodySender = mpsc::Sender<Result<Bytes, crate::hyper::Error>>;
...
enum Kind {
    ...
    Chan {
        ...
        data_rx: mpsc::Receiver<Result<Bytes, crate::hyper::Error>>,
...
impl Incoming {
    ...
    pub(crate) fn ffi() -> Incoming {
        Incoming::new(Kind::Ffi(crate::hyper::ffi::UserBody::new()))
    }
    ...
    #[cfg(feature = "ffi")]
    pub(crate) fn as_ffi_mut(&mut self) -> &mut crate::hyper::ffi::UserBody {
        match self.kind {
            Kind::Ffi(ref mut body) => return body,
            _ => {
                self.kind = Kind::Ffi(crate::hyper::ffi::UserBody::new());
            }
        }
        ...
        match self.kind {
            ...
            Kind::H2 {
                    ...
                    match ready!(h2.poll_data(cx)) {
                        ...
                        Some(Err(e)) => {
                            return match e.reason() {
                                ...
                                _ => Poll::Ready(Some(Err(crate::hyper::Error::new_body(e)))),
                            };
                        }
                        None => {
                            *data_done = true;
                ...
                match ready!(h2.poll_trailers(cx)) {
                    ...
                    Err(e) => Poll::Ready(Some(Err(crate::hyper::Error::new_h2(e)))),
                }
            }
...
impl Sender {
    ...
    pub(crate) fn poll_ready(
        ...
        self.data_tx
            .poll_ready(cx)
            .map_err(|_| crate::hyper::Error::new_closed())
    }

    fn poll_want(&mut self, cx: &mut task::Context<'_>) -> Poll<crate::hyper::Result<()>> {
        match self.want_rx.load(cx) {
            WANT_READY => Poll::Ready(Ok(())),
            WANT_PENDING => Poll::Pending,
            watch::CLOSED => Poll::Ready(Err(crate::hyper::Error::new_closed())),
            unexpected => unreachable!("want_rx value: {}", unexpected),
        }
    }

    async fn ready(&mut self) -> crate::hyper::Result<()> {
        futures_util::future::poll_fn(|cx| self.poll_ready(cx)).await
    }

    /// Send data on data channel when it is ready.
    #[allow(unused)]
    pub(crate) async fn send_data(&mut self, chunk: Bytes) -> crate::hyper::Result<()> {
        self.ready().await?;
        self.data_tx
            .try_send(Ok(chunk))
            .map_err(|_| crate::hyper::Error::new_closed())
    ...
    #[allow(unused)]
    pub(crate) async fn send_trailers(&mut self, trailers: HeaderMap) -> crate::hyper::Result<()> {
        let tx = match self.trailers_tx.take() {
            Some(tx) => tx,
            None => return Err(crate::hyper::Error::new_closed()),
        };
        tx.send(trailers)
            .map_err(|_| crate::hyper::Error::new_closed())
    }
    ...
    pub(crate) fn abort(mut self) {
        self.send_error(crate::hyper::Error::new_body_write_aborted());
    }

    pub(crate) fn send_error(&mut self, err: crate::hyper::Error) {
        let _ = self
            .data_tx
