...
use crate::hyper::common::{task, Future, Pin, Poll};
use crate::hyper::proto;
use crate::hyper::rt::bounds::ExecutorClient;
use crate::hyper::rt::Timer;
...
/// See [`client::conn`](crate::hyper::client::conn) for more.
pub async fn handshake<E, T, B>(
    exec: E,
    io: T,
) -> crate::hyper::Result<(SendRequest<B>, Connection<T, B, E>)>
where
...
impl<B> SendRequest<B> {
    ...
    pub fn poll_ready(&mut self, _cx: &mut task::Context<'_>) -> Poll<crate::hyper::Result<()>> {
        if self.is_closed() {
            Poll::Ready(Err(crate::hyper::Error::new_closed()))
        } else {
            Poll::Ready(Ok(()))
        }
    }
    ...
    ///
    /// If the associated connection is closed, this returns an Error.
    pub async fn ready(&mut self) -> crate::hyper::Result<()> {
    ...
    B: Body + 'static,
{
    ...
    pub fn send_request(
        &mut self,
        req: Request<B>,
    ) -> impl Future<Output = crate::hyper::Result<Response<IncomingBody>>> {
        ...
        async move {
            match sent {
                ...
                Err(_req) => {
                    ...
                    Err(crate::hyper::Error::new_canceled().with("connection was not ready"))
                }
            }
        }
    }
    ...
    pub(super) fn send_request_retryable(
        &mut self,
        req: Request<B>,
    ) -> impl Future<Output = Result<Response<Body>, (crate::hyper::Error, Option<Request<B>>)>> + Unpin
    where
        B: Send,
    {
        match self.dispatch.try_send(req) {
            ...
            Err(req) => {
                tracing::debug!("connection was not ready");
                let err = crate::hyper::Error::new_canceled().with("connection was not ready");
...
{
    type Output = crate::hyper::Result<()>;
    ...
    Ex: Clone,
{
    ...
    pub fn handshake<T, B>(
        &self,
        io: T,
    ) -> impl Future<Output = crate::hyper::Result<(SendRequest<B>, Connection<T, B, Ex>)>>
    where
        T: Read + Write + Unpin + 'static,
        B: Body + 'static,
        B::Data: Send,
