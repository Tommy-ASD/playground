...
#[cfg(feature = "http2")]
use crate::hyper::{common::Pin, proto::h2::client::ResponseFutMap};

#[cfg(test)]
pub(crate) type RetryPromise<T, U> = oneshot::Receiver<Result<U, (crate::hyper::Error, Option<T>)>>;
pub(crate) type Promise<T> = oneshot::Receiver<Result<T, crate::hyper::Error>>;
...
impl<T, U> Drop for Envelope<T, U> {
    fn drop(&mut self) {
        if let Some((val, cb)) = self.0.take() {
            cb.send(Err((
                crate::hyper::Error::new_canceled().with("connection closed"),
                Some(val),
            )));
        }
    }
...
pub(crate) enum Callback<T, U> {
    #[allow(unused)]
    Retry(Option<oneshot::Sender<Result<U, (crate::hyper::Error, Option<T>)>>>),
    NoRetry(Option<oneshot::Sender<Result<U, crate::hyper::Error>>>),
}
...
impl<T, U> Drop for Callback<T, U> {
    fn drop(&mut self) {
        ...
        let error =
            crate::hyper::Error::new_user_dispatch_gone().with(if std::thread::panicking() {
                "user code panicked"
            } else {
...
impl<T, U> Callback<T, U> {
    ...
    pub(crate) fn send(mut self, val: Result<U, (crate::hyper::Error, Option<T>)>) {
        match self {
