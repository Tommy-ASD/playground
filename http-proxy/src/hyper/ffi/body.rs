...
use crate::hyper::body::{Bytes, Frame, Incoming as IncomingBody};
            ...
            while let Some(item) = body.0.frame().await {
                ...
                if let Ok(chunk) = frame.into_data() {
                    if HYPER_ITER_CONTINUE != func(userdata.0, &hyper_buf(chunk)) {
                        return Err(crate::hyper::Error::new_user_aborted_by_callback());
                    }
                }
            }
            Ok(())
...
impl UserBody {
    ...
    pub(crate) fn poll_data(
        &mut self,
        cx: &mut Context<'_>,
    ) -> Poll<Option<crate::hyper::Result<Frame<Bytes>>>> {
        ...
        match (self.data_func)(self.userdata, hyper_context::wrap(cx), &mut out) {
            ...
            super::task::HYPER_POLL_ERROR => {
                Poll::Ready(Some(Err(crate::hyper::Error::new_body_write_aborted())))
            }
            unexpected => Poll::Ready(Some(Err(crate::hyper::Error::new_body_write(format!(
