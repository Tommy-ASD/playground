...
use crate::hyper::body::DecodedLength;
#[cfg(feature = "server")]
use crate::hyper::common::time::Time;
use crate::hyper::common::{task, Pin, Poll, Unpin};
use crate::hyper::headers::connection_keep_alive;
use crate::hyper::proto::{BodyLength, MessageHead};
#[cfg(feature = "server")]
use crate::hyper::rt::Sleep;
    ...
    T: Http1Transaction,
{
    ...
    pub(crate) fn pending_upgrade(&mut self) -> Option<crate::hyper::upgrade::Pending> {
    ...
    ) -> Poll<Option<crate::hyper::Result<(MessageHead<T::Incoming>, DecodedLength, Wants)>>> {
    ...
    fn on_read_head_error<Z>(
        &mut self,
        e: crate::hyper::Error,
    ) -> Poll<Option<crate::hyper::Result<Z>>> {
    ...
    // state.
    fn require_empty_read(&mut self, cx: &mut task::Context<'_>) -> Poll<crate::hyper::Result<()>> {
        ...
        if !self.io.read_buf().is_empty() {
            ...
            return Poll::Ready(Err(crate::hyper::Error::new_unexpected_message()));
        }

        let num_read = ready!(self.force_io_read(cx)).map_err(crate::hyper::Error::new_io)?;

        if num_read == 0 {
            let ret = if self.should_error_on_eof() {
                trace!("found unexpected EOF on busy connection: {:?}", self.state);
                Poll::Ready(Err(crate::hyper::Error::new_incomplete()))
            } else {
        ...
        Poll::Ready(Err(crate::hyper::Error::new_unexpected_message()))
    }
            ...
            if self.io.read_buf().is_empty() {
                match self.io.poll_read_from_io(cx) {
                    ...
                    Poll::Ready(Err(e)) => {
                        ...
                        self.state.close();
                        self.state.error = Some(crate::hyper::Error::new_io(e));
                    }
                }
            }
            ...
            Ok(encoder) => {
                ...
                #[cfg(feature = "ffi")]
                {
                    self.state.on_informational = head
                        .extensions
                        .remove::<crate::hyper::ffi::OnInformational>();
                }
    ...
    pub(crate) fn end_body(&mut self) -> crate::hyper::Result<()> {
        ...
        match encoder.end() {
            ...
            Err(not_eof) => {
                self.state.writing = Writing::Closed;
                Err(crate::hyper::Error::new_body_write_aborted().with(not_eof))
            }
        }
    }
    ...
    fn on_parse_error(&mut self, err: crate::hyper::Error) -> crate::hyper::Result<()> {
        if let Writing::Init = self.state.writing {
            if self.has_h2_prefix() {
                return Err(crate::hyper::Error::new_version_h2());
            }
    ...
    pub(crate) fn take_error(&mut self) -> crate::hyper::Result<()> {
    ...
    pub(super) fn on_upgrade(&mut self) -> crate::hyper::upgrade::OnUpgrade {
...
struct State {
    ...
    error: Option<crate::hyper::Error>,
    ...
    /// received.
    #[cfg(feature = "ffi")]
    on_informational: Option<crate::hyper::ffi::OnInformational>,
    ...
    upgrade: Option<crate::hyper::upgrade::Pending>,
...
impl State {
    ...
    fn prepare_upgrade(&mut self) -> crate::hyper::upgrade::OnUpgrade {
        let (tx, rx) = crate::hyper::upgrade::pending();
...
mod tests {
    ...
    fn bench_read_head_short(b: &mut ::test::Bencher) {
        ...
        let mut conn = Conn::<_, bytes::Bytes, crate::hyper::proto::h1::ServerTransaction>::new(io);
