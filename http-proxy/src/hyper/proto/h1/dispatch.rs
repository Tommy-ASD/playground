...
use crate::hyper::rt::{Read, Write};
use bytes::{Buf, Bytes};
use http::Request;
use tracing::{debug, trace};

use super::{Http1Transaction, Wants};
use crate::hyper::body::{Body, DecodedLength, Incoming as IncomingBody};
use crate::hyper::common::{task, Future, Pin, Poll, Unpin};
use crate::hyper::proto::{BodyLength, Conn, Dispatched, MessageHead, RequestHead};
use crate::hyper::upgrade::OnUpgrade;

pub(crate) struct Dispatcher<D, Bs: Body, I, T> {
    conn: Conn<I, Bs::Data, T>,
    dispatch: D,
    body_tx: Option<crate::hyper::body::Sender>,
...
pub(crate) trait Dispatch {
    ...
    fn recv_msg(
        &mut self,
        msg: crate::hyper::Result<(Self::RecvItem, IncomingBody)>,
    ) -> crate::hyper::Result<()>;
...
cfg_server! {
    use crate::hyper::service::HttpService;
...
cfg_client! {
    pin_project_lite::pin_project! {
        pub(crate) struct Client<B> {
            callback: Option<crate::hyper::client::dispatch::Callback<Request<B>, http::Response<IncomingBody>>>,
            #[pin]
            rx: ClientRx<B>,
            rx_closed: bool,
        }
    ...
    type ClientRx<B> = crate::hyper::client::dispatch::Receiver<Request<B>, http::Response<IncomingBody>>;
}
    ...
    Bs::Error: Into<Box<dyn StdError + Send + Sync>>,
{
    ...
    fn poll_inner(
        ...
        if self.is_done() {
            if let Some(pending) = self.conn.pending_upgrade() {
                ...
                ready!(self.conn.poll_shutdown(cx)).map_err(crate::hyper::Error::new_shutdown)?;
            }
            self.conn.take_error()?;
    ...
    fn poll_loop(&mut self, cx: &mut task::Context<'_>) -> Poll<crate::hyper::Result<()>> {
    ...
    fn poll_read(&mut self, cx: &mut task::Context<'_>) -> Poll<crate::hyper::Result<()>> {
        loop {
            if self.is_closing {
                return Poll::Ready(Ok(()));
                ...
                if self.conn.can_read_body() {
                    ...
                    match self.conn.poll_read_body(cx) {
                        ...
                        Poll::Ready(Some(Err(e))) => {
                            body.send_error(crate::hyper::Error::new_body(e));
                        }
                    }
                } else {
                    // just drop, the body will close automatically
    ...
    fn poll_read_head(&mut self, cx: &mut task::Context<'_>) -> Poll<crate::hyper::Result<()>> {
    ...
    fn poll_write(&mut self, cx: &mut task::Context<'_>) -> Poll<crate::hyper::Result<()>> {
        loop {
            if self.is_closing {
                return Poll::Ready(Ok(()));
                ...
                && self.dispatch.should_poll()
            {
                if let Some(msg) = ready!(Pin::new(&mut self.dispatch).poll_msg(cx)) {
                    let (head, body) = msg.map_err(crate::hyper::Error::new_user_service)?;
                    ...
                    OptGuard::new(self.body_rx.as_mut()).guard_mut()
                {
                    ...
                    if let Some(item) = item {
                        let frame = item.map_err(|e| {
                            *clear_body = true;
                            crate::hyper::Error::new_user_body(e)
                        })?;
    ...
    fn poll_flush(&mut self, cx: &mut task::Context<'_>) -> Poll<crate::hyper::Result<()>> {
        self.conn.poll_flush(cx).map_err(|err| {
            debug!("error writing: {}", err);
            crate::hyper::Error::new_body_write(err)
        })
    }
...
where
    ...
    Bs::Error: Into<Box<dyn StdError + Send + Sync>>,
{
    type Output = crate::hyper::Result<Dispatched>;
...
cfg_server! {
        ...
        Bs: Body,
    {
        ...
        fn recv_msg(&mut self, msg: crate::hyper::Result<(Self::RecvItem, IncomingBody)>) -> crate::hyper::Result<()> {
            let (msg, body) = msg?;
...
cfg_client! {
        ...
        B: Body,
    {
        ...
        type PollBody = B;
        type PollError = crate::hyper::common::Never;
        type RecvItem = crate::hyper::proto::ResponseHead;

        fn poll_msg(
            mut self: Pin<&mut Self>,
            cx: &mut task::Context<'_>,
        ) -> Poll<Option<Result<(Self::PollItem, Self::PollBody), crate::hyper::common::Never>>> {
            let mut this = self.as_mut();
                    ...
                    match cb.poll_canceled(cx) {
                        ...
                        Poll::Pending => {
                            ...
                            let head = RequestHead {
                                version: parts.version,
                                subject: crate::hyper::proto::RequestLine(parts.method, parts.uri),
                                headers: parts.headers,
                                extensions: parts.extensions,
                            };
            ...
            match msg {
                Ok((msg, body)) => {
                    if let Some(cb) = self.callback.take() {
                        ...
                        // full message!
                        Err(crate::hyper::Error::new_unexpected_message())
                    }
                }
                Err(err) => {
                    if let Some(cb) = self.callback.take() {
                        ...
                        if let Some((req, cb)) = self.rx.try_recv() {
                            ...
                            cb.send(Err((crate::hyper::Error::new_canceled().with(err), Some(req))));
                            Ok(())
                        } else {
                            Err(err)
                        }
...
mod tests {
    use super::*;
    use crate::hyper::common::io::compat;
    use crate::hyper::proto::h1::ClientTransaction;
    ...
    fn client_read_bytes_before_writing_request() {
        ...
        tokio_test::task::spawn(()).enter(|cx, _| {
            ...
            let (mut tx, rx) = crate::hyper::client::dispatch::channel();
            ...
            let mut res_rx = tx
                .try_send(crate::hyper::Request::new(IncomingBody::empty()))
                .unwrap();
            ...
            match (err.0.kind(), err.1) {
                (&crate::hyper::error::Kind::Canceled, Some(_)) => (),
    ...
    async fn client_flushing_is_not_ready_for_next_request() {
        ...
        let req = crate::hyper::Request::builder()
            .method("POST")
            .body(body)
            .unwrap();
    ...
    async fn body_empty_chunks_ignored() {
        ...
        let _res_rx = tx.try_send(crate::hyper::Request::new(body)).unwrap();
