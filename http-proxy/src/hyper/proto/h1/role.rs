...
#[cfg(feature = "server")]
use crate::hyper::common::date;
use crate::hyper::error::Parse;
use crate::hyper::ext::HeaderCaseMap;
#[cfg(feature = "ffi")]
use crate::hyper::ext::OriginalHeaderOrder;
use crate::hyper::headers;
use crate::hyper::proto::h1::{
    Encode, Encoder, Http1Transaction, ParseContext, ParseResult, ParsedMessage,
};
use crate::hyper::proto::{BodyLength, MessageHead, RequestHead, RequestLine};
...
impl Http1Transaction for Server {
    ...
    fn encode(
        ...
        let (ret, is_last) = if msg.head.subject == StatusCode::SWITCHING_PROTOCOLS {
            ...
            *msg.head = MessageHead::default();
            ...
            msg.body = None;
            (
                Err(crate::hyper::Error::new_user_unsupported_status_code()),
                true,
            )
        } else {
            (Ok(()), !msg.keep_alive)
        ...
        let custom_reason_phrase = msg.head.extensions.get::<crate::hyper::ext::ReasonPhrase>();
    ...
    fn on_error(err: &crate::hyper::Error) -> Option<MessageHead<Self::Outgoing>> {
        use crate::hyper::error::Kind;
            ...
            match *name {
                header::CONTENT_LENGTH => {
                    if wrote_len && !is_name_written {
                        ...
                        rewind(dst);
                        return Err(crate::hyper::Error::new_user_header());
                    }
                    match msg.body {
...
impl Http1Transaction for Client {
    ...
    fn parse(buf: &mut BytesMut, ctx: ParseContext<'_>) -> ParseResult<StatusCode> {
        ...
        loop {
            ...
            if let Some(reason) = reason {
                ...
                // field.
                let reason = crate::hyper::ext::ReasonPhrase::from_bytes_unchecked(reason);
            ...
            if head.subject.is_informational() {
                if let Some(callback) = ctx.on_informational {
                    callback.call(head.into_response(crate::hyper::body::Incoming::empty()));
                }
            }
    ...
    fn encode(msg: Encode<'_, Self::Outgoing>, dst: &mut Vec<u8>) -> crate::hyper::Result<Encoder> {
        trace!(
    ...
    fn on_error(_err: &crate::hyper::Error) -> Option<MessageHead<Self::Outgoing>> {
...
mod tests {
    ...
    fn test_parse_request() {
        ...
        .unwrap();
        assert_eq!(raw.len(), 0);
        assert_eq!(msg.head.subject.0, crate::hyper::Method::GET);
        assert_eq!(msg.head.subject.1, "/echo");
        assert_eq!(msg.head.version, crate::hyper::Version::HTTP_11);
        assert_eq!(msg.head.headers.len(), 1);
        assert_eq!(msg.head.headers["Host"], "hyper.rs");
        assert_eq!(method, Some(crate::hyper::Method::GET));
    }
    ...
    fn test_parse_response() {
        ...
        let ctx = ParseContext {
            cached_headers: &mut None,
            req_method: &mut Some(crate::hyper::Method::GET),
            h1_parser_config: Default::default(),
            h1_header_read_timeout: None,
            h1_header_read_timeout_fut: &mut None,
            h1_header_read_timeout_running: &mut false,
        ...
        assert_eq!(raw.len(), 0);
        assert_eq!(msg.head.subject, crate::hyper::StatusCode::OK);
        assert_eq!(msg.head.version, crate::hyper::Version::HTTP_11);
        assert_eq!(msg.head.headers.len(), 1);
    ...
    fn test_parse_response_h09_allowed() {
        ...
        assert_eq!(msg.head.subject, crate::hyper::StatusCode::OK);
        assert_eq!(msg.head.version, crate::hyper::Version::HTTP_09);
    ...
    fn test_decoder_request() {
        ...
        fn parse_err(s: &str, comment: &str) -> crate::hyper::error::Parse {
            let mut bytes = BytesMut::from(s);
            Server::parse(
                &mut bytes,
                ParseContext {
    ...
    fn test_decoder_response() {
        ...
        fn parse_err(s: &str) -> crate::hyper::error::Parse {
            let mut bytes = BytesMut::from(s);
            Client::parse(
                &mut bytes,
                ParseContext {
    ...
    fn test_client_request_encode_title_case() {
        use crate::hyper::proto::BodyLength;
