...
use crate::hyper::client::dispatch::{Callback, SendWhen};
use crate::hyper::common::io::Compat;
use crate::hyper::common::time::Time;
use crate::hyper::common::{task, Future, Never, Pin, Poll};
use crate::hyper::ext::Protocol;
use crate::hyper::headers;
use crate::hyper::proto::h2::UpgradedSendStream;
use crate::hyper::proto::Dispatched;
use crate::hyper::rt::bounds::ExecutorClient;
use crate::hyper::upgrade::Upgraded;
use crate::hyper::{Request, Response};
use h2::client::ResponseFuture;

type ClientRx<B> = crate::hyper::client::dispatch::Receiver<Request<B>, Response<IncomingBody>>;
...
pub(crate) async fn handshake<T, B, E>(
    ...
    mut exec: E,
    timer: Time,
) -> crate::hyper::Result<ClientTask<B, E, T>>
where
    ...
    B::Error: Into<Box<dyn std::error::Error + Send + Sync>>,
{
    let (h2_tx, mut conn) = new_builder(config)
        .handshake::<_, SendBuf<B::Data>>(crate::hyper::common::io::compat(io))
        .await
        .map_err(crate::hyper::hyper::Error::new_h2)?;
...
{
    type Output =
        Result<Response<crate::hyper::body::Incoming>, (crate::hyper::Error, Option<Request<B>>)>;
    ...
    fn poll(self: Pin<&mut Self>, cx: &mut std::task::Context<'_>) -> Poll<Self::Output> {
        ...
        match result {
                ...
                if let (Some(mut send_stream), StatusCode::OK) = (send_stream, res.status()) {
                    if content_length.map_or(false, |len| len != 0) {
                        ...
                        return Poll::Ready(Err((
                            crate::hyper::Error::new_h2(h2::Reason::INTERNAL_ERROR.into()),
                            None::<Request<B>>,
                        )));
                    }
                    ...
                    let (pending, on_upgrade) = crate::hyper::upgrade::pending();
                    let io = H2Upgraded {
                        ping: ping,
            ...
            Err(err) => {
                ...
                Poll::Ready(Err((crate::hyper::Error::new_h2(err), None::<Request<B>>)))
            }
        }
    }
}
            ...
            match ready!(self.h2_tx.poll_ready(cx)) {
                ...
                Err(err) => {
                    ...
                    return if err.reason() == Some(::h2::Reason::NO_ERROR) {
                        ...
                        Poll::Ready(Err(crate::hyper::Error::new_h2(err)))
                    };
                }
            };
                ...
                Poll::Ready(Some((req, cb))) => {
                    ...
                    let (fut, body_tx) = match self.h2_tx.send_request(req, !is_connect && eos) {
                        ...
                        Err(err) => {
                            ...
                            cb.send(Err((crate::hyper::Error::new_h2(err), None)));
                            continue;
                        }
                    };
                    ...
                    match self.h2_tx.poll_ready(cx) {
                        ...
                        Poll::Ready(Ok(())) => (),
                        Poll::Ready(Err(err)) => {
                            f.cb.send(Err((crate::hyper::Error::new_h2(err), None)));
                            continue;
                        }
                    }
                    self.poll_pipe(f, cx);
