...
use crate::hyper::body::{Body, Incoming as IncomingBody};
use crate::hyper::common::time::Time;
use crate::hyper::common::{date, task, Future, Pin, Poll};
use crate::hyper::ext::Protocol;
use crate::hyper::headers;
use crate::hyper::proto::h2::ping::Recorder;
use crate::hyper::proto::h2::{H2Upgraded, UpgradedSendStream};
use crate::hyper::proto::Dispatched;
use crate::hyper::rt::bounds::Http2ConnExec;
use crate::hyper::service::HttpService;

use crate::hyper::upgrade::{OnUpgrade, Pending, Upgraded};
    ...
    B: Body,
{
    Handshaking {
        ...
        hs: Handshake<crate::hyper::common::io::Compat<T>, SendBuf<B::Data>>,
    },
    Serving(Serving<T, B>),
    Closed,
}
    ...
    B: Body,
{
    ...
    conn: Connection<crate::hyper::common::io::Compat<T>, SendBuf<B::Data>>,
    closing: Option<crate::hyper::Error>,
}
    ...
    E: Http2ConnExec<S::Future, B>,
{
    pub(crate) fn new(
        ...
        let handshake = builder.handshake(crate::hyper::common::io::compat(io));
        ...
        loop {
            let next = match me.state {
                State::Handshaking {
                    ...
                    let mut conn =
                        ready!(Pin::new(hs).poll(cx).map_err(crate::hyper::Error::new_h2))?;
    ...
    B: Body + 'static,
{
        ...
        E: Http2ConnExec<S::Future, B>,
    {
                ...
                match ready!(self.conn.poll_accept(cx)) {
                    Some(Ok((req, mut respond))) => {
                        ...
                        let (mut req, connect_parts) = if !is_connect {
                            ...
                            let (pending, upgrade) = crate::hyper::upgrade::pending();
                    ...
                    Some(Err(e)) => {
                        return Poll::Ready(Err(crate::hyper::Error::new_h2(e)));
                    }
                    None => {
        ...
        ready!(self
            .conn
            .poll_closed(cx)
            .map_err(crate::hyper::Error::new_h2))?;
...
{
    fn poll2(self: Pin<&mut Self>, cx: &mut task::Context<'_>) -> Poll<crate::hyper::Result<()>> {
                    ...
                    let res = match h.poll(cx) {
                        ...
                        Poll::Pending => {
                                ...
                                .map_err(crate::hyper::Error::new_h2)?
                            {
                                ...
                                return Poll::Ready(Err(crate::hyper::Error::new_h2(
                                    reason.into(),
                                )));
                            }
                            return Poll::Pending;
                        ...
                        Poll::Ready(Err(e)) => {
                            let err = crate::hyper::Error::new_user_service(e);
                    ...
                    if let Some(connect_parts) = connect_parts.take() {
                        if res.status().is_success() {
                                ...
                                .map_or(false, |len| len != 0)
                            {
                                ...
                                return Poll::Ready(Err(crate::hyper::Error::new_user_header()));
                            }
