...
use crate::upgrade::common::Future;
use crate::upgrade::common::{task, watch, Pin, Poll};

type BodySender = mpsc::Sender<Result<Bytes, crate::upgrade::error::Error>>;
...
enum Kind {
    ...
    Chan {
        ...
        data_rx: mpsc::Receiver<Result<Bytes, crate::upgrade::error::Error>>,
...
impl Body for Incoming {
    type Data = Bytes;
    type Error = crate::upgrade::error::Error;
        ...
        match self.kind {
            ...
            Kind::H2 {
                        ...
                        Some(Err(e)) => {
                            return match e.reason() {
                                ...
                                _ => Poll::Ready(Some(Err(
                                    crate::upgrade::error::Error::new_body(e),
                                ))),
                            };
                        }
                        None => {
                ...
                match ready!(h2.poll_trailers(cx)) {
                    ...
                    Err(e) => Poll::Ready(Some(Err(crate::upgrade::error::Error::new_h2(e)))),
                }
            }
...
impl Sender {
    ...
    pub(crate) fn poll_ready(
        &mut self,
        cx: &mut task::Context<'_>,
    ) -> Poll<crate::upgrade::error::Result<()>> {
        // Check if the receiver end has tried polling for the body yet
        ready!(self.poll_want(cx)?);
        self.data_tx
            .poll_ready(cx)
            .map_err(|_| crate::upgrade::error::Error::new_closed())
    }

    fn poll_want(&mut self, cx: &mut task::Context<'_>) -> Poll<crate::upgrade::error::Result<()>> {
        match self.want_rx.load(cx) {
            WANT_READY => Poll::Ready(Ok(())),
            WANT_PENDING => Poll::Pending,
            watch::CLOSED => Poll::Ready(Err(crate::upgrade::error::Error::new_closed())),
            unexpected => unreachable!("want_rx value: {}", unexpected),
        }
    }

    async fn ready(&mut self) -> crate::upgrade::error::Result<()> {
        futures_util::future::poll_fn(|cx| self.poll_ready(cx)).await
    }

    /// Send data on data channel when it is ready.
    #[allow(unused)]
    pub(crate) async fn send_data(&mut self, chunk: Bytes) -> crate::upgrade::error::Result<()> {
        self.ready().await?;
        self.data_tx
            .try_send(Ok(chunk))
            .map_err(|_| crate::upgrade::error::Error::new_closed())
    ...
    pub(crate) async fn send_trailers(
        &mut self,
        trailers: HeaderMap,
    ) -> crate::upgrade::error::Result<()> {
        let tx = match self.trailers_tx.take() {
            Some(tx) => tx,
            None => return Err(crate::upgrade::error::Error::new_closed()),
        };
        tx.send(trailers)
            .map_err(|_| crate::upgrade::error::Error::new_closed())
    }
    ...
    pub(crate) fn abort(mut self) {
        self.send_error(crate::upgrade::error::Error::new_body_write_aborted());
    }

    pub(crate) fn send_error(&mut self, err: crate::upgrade::error::Error) {
        let _ = self
            .data_tx
