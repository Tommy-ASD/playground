...
use crate::upgrade::common::io::Rewind;
use crate::upgrade::common::{task, Future, Pin, Poll};
...
pub struct OnUpgrade {
    rx: Option<oneshot::Receiver<crate::upgrade::error::Result<Upgraded>>>,
}
...
impl crate::bidirectional::io::AsyncRead for Upgraded {
    fn poll_read(
        mut self: Pin<&mut Self>,
        cx: &mut task::Context<'_>,
        buf: &mut crate::bidirectional::io::ReadBuf<'_>,
    ) -> Poll<io::Result<()>> {
        crate::bidirectional::io::AsyncRead::poll_read(Pin::new(&mut self.io), cx, buf)
    }
}
...
impl crate::bidirectional::io::AsyncWrite for Upgraded {
    fn poll_write(
        mut self: Pin<&mut Self>,
        cx: &mut task::Context<'_>,
        buf: &[u8],
...
impl Future for OnUpgrade {
    type Output = Result<Upgraded, crate::upgrade::error::Error>;
        ...
        match self.rx {
            Some(ref mut rx) => Pin::new(rx).poll(cx).map(|res| match res {
                ...
                Err(_oneshot_canceled) => {
                    Err(crate::upgrade::error::Error::new_canceled().with(UpgradeExpected))
                }
            }),
            None => Poll::Ready(Err(crate::upgrade::error::Error::new_user_no_upgrade())),
        }
    }
}
