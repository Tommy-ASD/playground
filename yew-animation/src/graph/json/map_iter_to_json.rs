...
pub trait MapIterToJson<'a, K, V>: IntoIterator<Item = (&'a K, &'a V)>
where
    Self: Sized,
    K: 'a + Serialize + Any,
    V: 'a + Serialize,
...
pub(crate) struct SerializeMapIterWrapper<'a, K, V, I>
where
...
impl<'a, K, V, I> Serialize for SerializeMapIterWrapper<'a, K, V, I>
where
    I: Iterator<Item = (&'a K, &'a V)>,
    K: Serialize + Any,
    V: Serialize,
        ...
        S: Serializer,
    {
        ...
        if TypeId::of::<K>() == TypeId::of::<String>() {
            while let Some((k, v)) = iter.next() {
                let s = (k as &dyn Any)
                    .downcast_ref::<String>()
                    .ok_or(S::Error::custom("Failed to serialize String as string"))?;
            ...
            while let Some((k, v)) = iter.next() {
                ser_map.serialize_entry(
                    match &serde_json::to_string(&k) {
                        ...
                        Err(e) => {
                            return Err(e).map_err(S::Error::custom);
                        }
                    },
                    &v,
                )?;
